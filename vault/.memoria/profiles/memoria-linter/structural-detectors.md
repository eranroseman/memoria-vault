...
1. For each skeleton note (`start-here.md`, `system-status.md`, `safe-mode.md`, `schema-reference.md`), read its `updated_at` frontmatter. (The other former skeleton notes now live only on the website — see [docs](https://eranroseman.github.io/memoria-vault/) — and are no longer vault-resident, so they're out of skeleton-drift scope.)
...
3. **Cross-reference both directions.**
   ...
   - For each *declared* command in a SOUL.md: it should appear in at least one of the `docs/` profile command tables (`docs/explanation/profiles/README.md` lane matrix or `docs/reference/obsidian-command-palette.md`), unless explicitly noted as private to that profile.
