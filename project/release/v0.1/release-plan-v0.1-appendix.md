...
5. Confirm Zotero + Better BibTeX (citekey format `[auth.lower][year][title.lower:select,1,1]` per [ADR-6](../../decisions/06-citekey-naming-convention.md)) auto-exports to `.memoria/memoria.bib`.
...
1. **Choose a deployment pattern.** The adopted `local-only` default and the common conventions are in [deployment options](../../../docs/explanation/deployment/deployment-options.md); the multi-machine patterns and their full comparison are the [multi-machine deployment proposal](../../proposals/explorations/multi-machine-deployment.md). `local-mesh` (Syncthing, peer-to-peer) suits most setups; `always-on` (Syncthing + VPS) is for unattended overnight ingest. Pick based on whether the secondary device is always reachable.
