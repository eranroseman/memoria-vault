...
Every design component → the layer/plan that covers it → whether it's automated → status. This is the keystone of the [testing framework](../../docs/adr/29-testing-framework.md): if a surface isn't a row here, it isn't tracked. Update it whenever a component or plan changes.

**Layers** (see [ADR-29](../../docs/adr/29-testing-framework.md)): **L0** static+schema · **L1** component self-tests · **L2** agent wiring + policy gate · **L3** system/GUI · **L4** golden-path E2E · **L5** quality/eval · **X** cross-cutting.
...
| 6 | 7 profiles — every documented CLI command | L2 | [hermes-cli](plans/hermes-cli-test-plan.md) §4 | manual | ✅ |
...
- Framework + layer definitions: [ADR-29](../../docs/adr/29-testing-framework.md)
