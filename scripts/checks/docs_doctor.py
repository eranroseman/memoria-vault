...
REFERENCE_SOURCE_STATUSES = {"Manual", "Source-owned", "Guarded mirror", "Generated"}
...
def _markdown_code_symbols(text: str) -> set[str]:
    out: set[str] = set()
    for value in _markdown_code_values(text):
...
def check_reference_source_contract(repo: Path, errors: list[str]) -> None:
    ...
    if not contract_path.is_file():
        errors.append(f"{contract_path}: missing reference source-status contract")
        return
    try:
        data = yaml.safe_load(read(contract_path)) or {}
    except yaml.YAMLError as exc:
        errors.append(f"{contract_path}: invalid YAML: {exc}")
        return
    ...
    if missing:
        errors.append(f"{contract_path}: omits reference page(s): {', '.join(missing)}")
    if extra:
        errors.append(f"{contract_path}: lists unknown reference page(s): {', '.join(extra)}")

    readme_statuses = _reference_readme_statuses(repo)
    for name, raw in sorted(pages.items()):
        if not isinstance(raw, dict):
            errors.append(f"{contract_path}: {name} entry must be a mapping")
            continue
        status = str(raw.get("status") or "")
        owner = str(raw.get("owner") or "")
        check = str(raw.get("check") or "")
        if status not in REFERENCE_SOURCE_STATUSES:
            errors.append(f"{contract_path}: {name} has unknown status {status!r}")
        if not owner:
            errors.append(f"{contract_path}: {name} must name an owner")
        if status == "Guarded mirror" and not check:
            errors.append(f"{contract_path}: {name} guarded mirror must name a drift check")
        page = reference_dir / str(name)
        if status == "Generated" and page.is_file() and "<!-- Generated by" not in read(page):
            errors.append(f"{page}: source status is Generated but page lacks a generated marker")
        readme_status = readme_statuses.get(str(name))
        if readme_status and readme_status != status:
            errors.append(
                f"{repo / 'docs' / 'reference' / 'README.md'}: {name} status "
                f"{readme_status!r} differs from _sources.yml {status!r}"
            )


def _operation_manifest_ids(repo: Path, *, prompt_only: bool = False) -> set[str]:
    out: set[str] = set()
    ...
    for path in sorted(ops.glob("*.md")):
        ...
        try:
            ...
            continue
        if prompt_only and not all(frontmatter.get(key) for key in ("posture", "mode", "action")):
            continue
        out.add(str(frontmatter.get("operation_id") or path.stem))
    return out
...
def _public_defs(path: Path) -> set[str]:
    ...
    return {
        match.group(1)
        for match in re.finditer(r"^def ([A-Za-z_][A-Za-z0-9_]*)\(", read(path), re.MULTILINE)
...
def _require_doc_codes(doc: Path, expected: set[str], label: str, errors: list[str]) -> None:
    if not expected or not doc.is_file():
        return
    text = read(doc)
    documented = _markdown_code_values(text) | _markdown_code_symbols(text)
    missing = sorted(expected - documented)
    if missing:
        errors.append(f"{doc}: {label} roster omits: {', '.join(missing)}")
...
def check_source_of_truth_mirrors(repo: Path, errors: list[str]) -> None:
    check_reference_source_contract(repo, errors)
    ...
    check_reference_rosters(repo, errors)
