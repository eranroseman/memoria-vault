#!/usr/bin/env bash
# Verify every docs/ reference under vault-template/ resolves.
#
# vault-template/ ships to the runtime WITHOUT the docs/ tree, so vault files must
...
#
# Scans the whole vault-template/ tree — this is the CI backstop for the per-diff
...
done < <(find vault-template -type f \( -name '*.md' -o -name '*.py' -o -name '*.yaml' -o -name '*.yml' \) \
           -not -path 'vault-template/.obsidian/*')

if [ "$fail" -ne 0 ]; then
  echo "docs-links: FAILED — fix the references above (vault-template/ must use Pages URLs that resolve)."
  exit 1
fi
echo "docs-links: OK — all vault-template/ docs references resolve."
