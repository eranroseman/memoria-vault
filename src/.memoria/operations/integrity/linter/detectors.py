...
_DESIGN_SCAN_PREFIXES = (
    ...
    "system/", "notes/", "catalog/", "inbox/", "projects/",
    ".obsidian/snippets/",
)
...
def design_system_drift(vault: Path) -> list[Finding]:
    """Report visual-style drift against `.memoria/design-system.md` (#378).

    The spec is the source of truth; consumers are subordinate. This detector is
    intentionally narrow: authored Markdown and shipped CSS snippets, not bundled
    third-party plugin code. Findings are report-only and surface through the
    normal Linter JSON/cron path.
    """
