...
vault — it ends its card report with one fenced ``json`` block::
...
fields its workflow produces and omits the rest. The scorer reads the cards via
``hermes kanban list --json`` (or ``--from-json`` for tests/offline — the
board_export.py pattern) and computes, per task, only what its result makes
computable — **no fake scores**; a task with no result block is reported
``unscored``:
...
scorer prints that and appends nothing (an empty run is noise, not data).
    ...
    python eval_score.py --vault <path>                       # score the current quarter
    python eval_score.py --vault <path> --quarter previous    # the quarter before (cron)
