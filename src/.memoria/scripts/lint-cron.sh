...
# monitors between). Runs the structural detectors over the vault and the
# golden-copy drift check; findings surface in the vault's drift dashboards.
...
"{{PYTHON}}" "{{VAULT_PATH}}/.memoria/engines/linter/detectors.py" --vault "{{VAULT_PATH}}" >/dev/null || true
