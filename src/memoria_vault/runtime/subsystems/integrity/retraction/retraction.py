#!/usr/bin/env python3
"""retraction.py — the retraction sweep operation (ADR-46/D41).
...
most-authoritative first:
  ...
  1. Retraction Watch DB — the authoritative dataset, Crossref-owned and freely
     ...
     `--refresh` downloads/refreshes the CSV as a scheduled task.
...
def rw_csv_path() -> Path:
    raw = (
        ...
        or os.environ.get("MEMORIA_VAULT_PATH", "") + "/.memoria/data/retraction_watch.csv"
    )
