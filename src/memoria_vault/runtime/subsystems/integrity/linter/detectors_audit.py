...
def vault_hash_drift(vault: Path) -> list[Finding]:
    ...
    Walks `system/logs/audit.jsonl` (append-only forever, so one walk
...
def audit_log_size(vault: Path, max_mb: float = 50.0) -> list[Finding]:
    """Advisory size check on the append-only-forever audit log.
    ...
    return [
        Finding(
            ...
            f"-- append-only forever by design, so growth is "
