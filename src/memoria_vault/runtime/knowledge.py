...
def _canvas_from_nodes_edges(
    ...
    for index, edge in enumerate(edges_in):
        ...
        if not source or not target:
            # Reconcile discipline (U3 §6): quarantine-and-log the dirty row,
            # never silent-drop it. JSON Canvas refuses a dangling endpoint, so
            # the edge still cannot ship — but it is now observable.
            quarantined.append(
                {
    ...
    return {"nodes": nodes, "edges": edges}, quarantined
...
def write_project_argument_canvas(
    ...
    *,
    ...
    if commit:
        ...
        # Only when non-empty: a clean run is the ordinary case, and an empty
        # list here would add a row to every journal (and every golden).
        if report["quarantined_edges"]:
            run_event["quarantined_edges"] = report["quarantined_edges"]
        event = append_journal_event(vault, run_event, context=context)
    ...
    return {
        ...
        # File nodes only: the banner is chrome, and every existing consumer of
        # this count means "how many members made it onto the canvas".
        "node_count": sum(1 for node in canvas["nodes"] if node.get("type") == "file"),
        "edge_count": len(canvas["edges"]),
        "quarantined_edge_count": len(report["quarantined_edges"]),
        "event": event,
        "commit": commit_id,
    }
