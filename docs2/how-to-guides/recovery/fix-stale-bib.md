...
```bash
grep <citekey> vault/.memoria/library.bib
```
...
In Zotero: File → Export Library → Better BibLaTeX → overwrite `vault/.memoria/library.bib`. Or make any change to the Zotero library (add/remove a tag) to trigger the auto-export.
...
grep <citekey> vault/.memoria/library.bib   # confirm entry is now present
```
...
```bash
grep <citekey> vault/.memoria/library.bib        # entry present
git log --oneline vault/.memoria/library.bib | head -3   # recent commit timestamp
```
...
- Confirm the export path is the absolute path to `vault/.memoria/library.bib`
...
- Full failure-modes catalog: [how-to/operations/failure-modes.md](../../memoria-vault/docs/how-to/operations/failure-modes.md)
