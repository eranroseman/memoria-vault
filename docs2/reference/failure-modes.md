...
All known failure modes, sorted by severity. Each entry: symptom, severity, cause, and fix. For full Detect / Fix / Verify recipes on the most common failures see [how-to/operations/failure-modes.md](../../docs/how-to/operations/failure-modes.md).
...
| Card not progressing (`running` / `ready` / `blocked`) | MEDIUM | Worker crashed mid-claim, unresolved `assignee`, or human decision owed on `blocked` card | See full recipe in [how-to/operations/failure-modes.md#stuck-card](../../docs/how-to/operations/failure-modes.md). |
