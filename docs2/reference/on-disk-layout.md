...
Where every file lives across the two filesystem roots: the starter vault (versioned, distributable) and `.memoria/` (tooling layer). For the design rationale see [explanation/architecture/on-disk-layout.md](../../docs/explanation/architecture/on-disk-layout.md).

**Implementation note.** Items marked `# pending` are designed but not yet present in the v0.1 starter vault. Items marked `# deferred` are planned for a future version. See [implementation-status.md](../../docs/project/implementation-status.md) for build state.
...
├── install.sh                      # macOS/Linux — deferred (v0.2)
│
...
│   ├── memoria-librarian/
│   │   ├── SOUL.md                 # shipped
│   │   ├── config.yaml             # pending (v0.2 wiring)
│   │   ├── mcp.json                # pending (v0.2 wiring)
│   │   ├── distribution.yaml       # pending (v0.2 wiring)
│   │   ├── .env.EXAMPLE            # pending
│   │   ├── cron/
...
│   └── linter.yaml                 # shipped
├── mcp/
│   └── .keep                       # pending: policy_mcp.py + tasks_mcp.py + requirements.txt
├── csl/
...
└── tool-registry.yaml              # pending
```
---

## Vault skeleton: human-facing notes

Notes in `00-meta/` that ship with the starter vault for human reference. These are the plain-language counterpart to the technical contracts in `.memoria/profiles/*/SOUL.md` and the `docs/` design files.
...
| `00-meta/04-reference/obsidian-config.md` | Plugin inventory and load-bearing settings the human should not change. Mirrors `docs2/reference/plugins.md`. | Human (sync with plugin changes) |
...
**Drift discipline.** When the design changes (new profile, new lane rule, new schema field), the corresponding skeleton note must be updated. The Linter's `skeleton-drift` detector flags notes whose `updated` timestamp lags the corresponding design file.
...
**No spaces in vault file names.** Obsidian can handle them, but Hermes path handling and shell scripts do not. Use `kebab-case` throughout.
