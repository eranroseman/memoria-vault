...
Subscription publisher APIs are deliberately out of scope (open-access full text via S2ORC/CORE/PMC/arXiv/Unpaywall covers the OA slice). Deferred: heavier domain NLP (scispaCy, an RCT/PICO classifier) only if the embedding/zero-shot tag layer proves insufficient; the S2 Recommendations API and `/snippet/search` for discovery; an API-POST capture transport ([control-plane.md](../../docs/explanation/architecture/control-plane.md) ideal) once the Hermes API is stood up — the script front-end ships first.
...
- **Workflows affected:** [capture-and-ingest.md](../../docs/how-to-guides/sources/capture-and-ingest.md), [obsidian-command-palette.md](../../docs/reference/obsidian-command-palette.md).
- **Files affected:** `obsidian-paper-note/SKILL.md` (slims to *call the ingest tool* + judgments + gated write), a new `ingest_paper.py` (source of truth for API field lists, chain order, and merge rules — kept out of the ADR so it can change without re-deciding), [ingest.md](../../docs/reference/ingest.md) (type routing, fallback chains, extraction tiers, S2-not-GROBID), [frontmatter.md](../../docs/reference/frontmatter.md) / [note-types.md](../../docs/reference/note-types.md) (`captured` + `ingest_status`), `capture-from-zotero.js` (`curl` not `requestUrl`; the capture-intake log), the seed-tag vocabulary format (tags carry definitions).
- **Correction (delivery mechanism):** the worker **cannot run the pipeline as a script** — the Librarian's capability allowlist (ADR-27) disables `code_execution`/`terminal`/`file`. The deterministic spine is therefore delivered as an **MCP tool** (`ingest_pipeline` on the `memoria-ingest` server, `mcp/ingest_mcp.py`, wrapping `pipeline.run()`), reached the same way vault access and the policy gate are. The tool reads + computes only; the agent still fills the two holes and writes through the gated obsidian MCP. (The CLI entry points remain for cron/sweeps and offline use.)
- **Related decisions / Depends on:** [ADR-27](27-hermes-native-config-and-gate-enforcement.md), [ADR-28](28-write-gate-as-plugin.md) (the write gate), [ADR-16](16-adopt-on-demand-for-reviews.md).
