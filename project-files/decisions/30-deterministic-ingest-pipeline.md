...
- **Files affected:** `obsidian-paper-note/SKILL.md` (slims to script + judgments + gated write), a new `ingest_paper.py` (source of truth for API field lists, chain order, and merge rules — kept out of the ADR so it can change without re-deciding), [ingest.md](../../docs/reference/ingest.md) (type routing, fallback chains, extraction tiers, S2-not-GROBID), [frontmatter.md](../../docs/reference/frontmatter.md) / [note-types.md](../../docs/reference/note-types.md) (`captured` + `ingest_status`), `capture-from-zotero.js` (`curl` not `requestUrl`; the capture-intake log), the seed-tag vocabulary format (tags carry definitions).
