...
- **Sequencing is not decided here.** *When* a decision is implemented lives in the [release roadmap](../releases/v0.1/release-plan-v0.1.md), which can change independently of these decisions. Decisions record the choice; the timeline records the schedule — link to it rather than restating phase order, so a re-plan does not strand stale dates in these files.
