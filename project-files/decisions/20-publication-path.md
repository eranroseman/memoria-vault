...
- **Build order is now ordered by the paper, not by feature appeal.** The Verifier profile and a public vault-CiteME fixture are the critical path; everything else is downstream. The board-export cron (Phase 1 in the [timeline](../plans/timeline.md)) becomes a publication prerequisite, not just an observability nicety — without it the six-signal logs never populate.
...
- **n=1 operator data is accepted as a known weakness** of the later system-paper path, to be mitigated by detailed logging and a within-subject comparison arm (blocking vs. advisory review, see [proposal-31](../proposals/PROP-08-configurable-review-gate-mode.md)) rather than by more operators.
...
- **Files affected:** [reference/telemetry.md](../../docs/reference/telemetry.md) (the six-signal schemas), [plans/timeline.md](../plans/timeline.md) (step 8, six-signal capture; board-export cron is the prerequisite), [plans/implementation-status.md](../plans/implementation-status.md) (six-signal capture row).
...
- **Proposals:** [measurement-and-verification.md](../proposals/measurement-and-verification.md) (analysis harnesses, deferred); [proposal-31](../proposals/PROP-08-configurable-review-gate-mode.md) (the comparison arm).
