...
- **Gold set (vault).** Gold tasks live in `00-meta/05-eval/` as YAML; they become a dedicated note type only if the [expansion-threshold](../roadmap/README.md#expansion-threshold-discipline) <!-- TODO: reorg link — no unique new target for roadmap/README.md --> is tripped.
...
- Diagnostic-only means a capability regression won't auto-halt work; the human must notice it on the dashboard. This is intentional, per [success-metrics.md](../roadmap/success-metrics.md) <!-- TODO: reorg link — no unique new target for roadmap/success-metrics.md --> ("diagnostic, not contract").
...
- **Workflows affected:** [Verify](../../how-to/workflows/downstream/verify.md) <!-- TODO: reorg link — no unique new target for how-to/workflows/downstream/verify.md --> (the eval reuses `cite-check`); the maintenance/`lint` surface (the Linter scores + reports).
- **Files affected:** [roadmap/evaluation.md](../roadmap/evaluation.md) <!-- TODO: reorg link — no unique new target for roadmap/evaluation.md -->, [architecture/on-disk-layout.md](../../docs/reference/on-disk-layout.md) (`00-meta/05-eval/`, `00-meta/08-metrics/eval/`), the Linter's `M-detectors.md` and a dashboard (in the starter vault).
...
- **Source discussion:** [roadmap/evaluation.md](../roadmap/evaluation.md) <!-- TODO: reorg link — no unique new target for roadmap/evaluation.md --> (Observability + Integration); the `vault-eval` scaffold.
