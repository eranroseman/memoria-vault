...
5. **Verify the policy gate:** run `python .memoria/mcp/policy_mcp.py --self-test` (34 checks) and `python .memoria/mcp/policy_hook.py --self-test` (32 checks, including file-toolset gating + pre→post roundtrip). Both must pass before any profile is allowed to run against the production vault.
