...
- The starter vault ships secret-bearing files only as sanitized `.example` /
  `.EXAMPLE` siblings; real local environment files, provider config, and
  optional adapter secrets are `.gitignore`d. **Never** remove those ignore
  rules or commit the real files.
