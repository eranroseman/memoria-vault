from __future__ import annotations

import json
import shutil
import tomllib
from pathlib import Path

from memoria_vault.runtime.retrieval_substrate import (
    RETRIEVAL_SUBSTRATE_VERDICT,
    SELECTED_RETRIEVAL_SUBSTRATE,
    evaluate_fts5_fixture,
)
from memoria_vault.runtime.search_index import rebuild_checked_qmd_source

ROOT = Path(__file__).resolve().parent.parent
FIXTURE = ROOT / "tests/fixtures/retrieval-substrate-spike.json"


def test_alpha15_retrieval_substrate_verdict_is_committed() -> None:
    fixture = json.loads(FIXTURE.read_text(encoding="utf-8"))

    assert fixture["verdict"]["selected"] == "qmd"
    assert fixture["verdict"]["cleared_replacement_bar"] is False
    assert SELECTED_RETRIEVAL_SUBSTRATE == fixture["verdict"]["selected"]
    assert RETRIEVAL_SUBSTRATE_VERDICT["selected"] == fixture["verdict"]["selected"]
    assert RETRIEVAL_SUBSTRATE_VERDICT["cleared_replacement_bar"] is False


def test_sqlite_fts5_does_not_clear_qmd_replacement_bar() -> None:
    fixture = json.loads(FIXTURE.read_text(encoding="utf-8"))
    result = evaluate_fts5_fixture(fixture)

    assert result["available"] is True
    assert result["recall_by_slice"]["keyword"] == 1.0
    assert (
        result["recall_by_slice"]["semantic"] < fixture["expected_qmd_recall_by_slice"]["semantic"]
    )
    assert result["recall_by_slice"]["hybrid"] < fixture["expected_qmd_recall_by_slice"]["hybrid"]


def test_qmd_stays_packaged_until_vector_retrieval_is_declared() -> None:
    pyproject = tomllib.loads((ROOT / "pyproject.toml").read_text(encoding="utf-8"))
    dependencies = {
        dependency.split("[", 1)[0].split(">=", 1)[0].split("==", 1)[0]
        for dependency in pyproject["project"]["dependencies"]
    }
    installer = (ROOT / "scripts/install/runtime-tools.sh").read_text(encoding="utf-8")

    assert SELECTED_RETRIEVAL_SUBSTRATE == "qmd"
    assert "ensure_qmd" in installer
    assert dependencies.isdisjoint({"sentence-transformers", "sqlite-vec", "sqlite_vec"})


def test_qmd_verdict_matches_rebuild_manifest(tmp_path: Path) -> None:
    shutil.copytree(ROOT / "vault-template/.memoria/schemas", tmp_path / ".memoria/schemas")
    note = tmp_path / "knowledge/notes/checked.md"
    note.parent.mkdir(parents=True)
    note.write_text(
        "---\ntype: note\ncheck_status: checked\ntitle: checked\n---\nalpha\n",
        encoding="utf-8",
    )

    manifest = rebuild_checked_qmd_source(tmp_path)

    assert SELECTED_RETRIEVAL_SUBSTRATE == "qmd"
    assert manifest["backend"] == SELECTED_RETRIEVAL_SUBSTRATE
