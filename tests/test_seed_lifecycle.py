...
def test_repair_leaves_pi_modified_view_preferences(
    ...
    assert "inbox.base" not in output["repaired"]
    assert ".obsidian/graph.json" not in output["repaired"]
    assert ".memoria/config/providers.yaml" in output["repaired"]
...
def test_repair_reseeds_deleted_view_preferences(
    ...
    assert "inbox.base" in output["repaired"]


def test_repair_rejects_dynamic_canvas_symlink(
    tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    workspace = _init(tmp_path, capsys)
...
def test_repair_does_not_run_workspace_fsmonitor(
    tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    workspace = _init(tmp_path, capsys)
