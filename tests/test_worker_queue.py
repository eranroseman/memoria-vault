...
def test_worker_runs_capture_zotero_source_operation_jobs(tmp_path: Path) -> None:
    ...
    item = {
        ...
        "data": {
            ...
            "title": "Queued Zotero Source",
            "creators": [{"creatorType": "author", "firstName": "Morgan", "lastName": "Lin"}],
            "date": "2026",
            "DOI": "10.1000/queued.zotero",
            "abstractNote": "Queued Zotero capture fixture.",
            "extra": "Citation Key: lin2026queued",
        },
    }
    ...
    queued = enqueue_operation(
        ...
        payload={"zotero_item": item, "run_id": "capture-zotero"},
        idempotency_key="capture-zotero",
    )
    done = run_next_job(vault, machine="test-machine")
...
def test_worker_runs_capture_zotero_source_from_local_api_key(tmp_path: Path, monkeypatch) -> None:
    ...
    queued = enqueue_operation(
        ...
        payload={
            ...
            "timeout": 2,
            "run_id": "capture-zotero-fetch",
        },
        idempotency_key="capture-zotero-fetch",
    )
    done = run_next_job(vault, machine="test-machine")
