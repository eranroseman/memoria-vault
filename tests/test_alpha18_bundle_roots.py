"""Alpha.18 bundle-root and fulltext contract tests."""
...
def test_alpha18_bundle_roots_and_fulltext_schema() -> None:
    folders = schema.load_folders()
    types = schema.load_types()

    assert schema.bundle_roots(folders) == ("notes", "hubs", "projects", "digests", "fulltext")
    assert schema.home_for("digest", folders) == "digests"
    assert schema.home_for("fulltext", folders) == "fulltext"
    ...
    with state.connect(tmp_path) as conn:
        with pytest.raises(sqlite3.IntegrityError):
            conn.execute(
                ...
                ("fulltext/source-alpha.md", "fulltext"),
            )
