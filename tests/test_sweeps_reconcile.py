...
def test_reconcile_intake():
    def _run():
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            checks = [
                (
                    "log parses, bad line skipped",
                    set(recs) == {"stuck2024A", "done2024B", "ghost2024Z"},
                ),
                ("note_for finds on-disk note", note_for("stuck2024A", v) is not None),
                ("note_for misses ghost", note_for("ghost2024Z", v) is None),
                (
                    "reconcile flags only the orphan",
                    [e["citekey"] for e in rec["enqueued"]] == ["ghost2024Z"],
                ),
                (
                    "retry selects only captured+tier0",
                    [e["citekey"] for e in ret["enqueued"]] == ["stuck2024A"],
                ),
                (
                    "dry-run enqueues nothing live",
                    all(e["card"] == "DRY" for e in rec["enqueued"] + ret["enqueued"]),
                ),
            ]
