...
def test_projection_regeneration_policy_allows_agents_projection(tmp_path: Path) -> None:
    from memoria_vault.runtime.operations import load_operation_policy

    policy = load_operation_policy(tmp_path, "regenerate-tracked-projections")

    assert "AGENTS.md" in policy["allowed_paths"]
...
def test_worker_regenerate_indexes_refuses_index_symlink(tmp_path: Path) -> None:
    vault = workspace(tmp_path)
    outside = tmp_path / "outside-index.md"
    outside.write_text("outside sentinel\n", encoding="utf-8")
    (vault / "index.md").symlink_to(outside)
    ...
    assert (
        "workspace write target must not redirect through a symlink or junction: index.md"
        in (done["error"])
    )
...
def test_worker_regenerate_tracked_projections_refuses_agents_symlink(tmp_path: Path) -> None:
    vault = workspace(tmp_path)
    outside = tmp_path / "outside-agents.md"
    outside.write_text("outside sentinel\n", encoding="utf-8")
    (vault / "AGENTS.md").symlink_to(outside)
    ...
    assert done["status"] == "failed"
    assert (
        "workspace write target must not redirect through a symlink or junction: AGENTS.md"
        in (done["error"])
    )
    assert outside.read_text(encoding="utf-8") == "outside sentinel\n"
