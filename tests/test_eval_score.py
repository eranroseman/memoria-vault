...
def _result_card(task: str, quarter: str, **fields) -> dict:
    block = json.dumps({"vault_eval": "result", "task": task,
                        "quarter": quarter, **fields})
    return {"task_id": f"card-{task}", "status": "done",
...
def test_superseded_classification_matches_the_linter_detector(tmp_path):
    ...
    (src / "uses.md").write_text(
        ...
        encoding="utf-8")
    flagged = {f.message.split("[[")[1].split("]]")[0]
               for f in detectors.fama_exposure(v)}
...
def test_recall_at_k_hit_miss_and_window(tmp_path):
    ...
    hit = eval_score.score_task(task, {"retrieved": ["a", "vaswani2017attention"]},
                                catalog, set())
    ...
    k1 = eval_score.score_task(task, {"retrieved": ["a", "vaswani2017attention"]},
                               catalog, set(), k=1)
...
def test_recall_at_k_fraction_over_multiple_gold_citekeys(tmp_path):
    ...
    m = eval_score.score_task(task, {"retrieved": ["a2020", "x", "y"]},
                              eval_score.catalog_citekeys(v), set())
...
def test_fama_clean_and_exposure(tmp_path):
    ...
    catalog = eval_score.catalog_citekeys(v)
    dirty = eval_score.score_task({"references": []},
                                  {"claims": ["good-claim", "old-claim"]}, catalog, sup)
    assert dirty["fama_clean"] == 0.0
    assert dirty["fama_exposed"] == ["old-claim"]
    clean = eval_score.score_task({"references": []},
                                  {"claims": ["good-claim"]}, catalog, sup)
...
def test_no_inputs_means_no_metrics_not_fake_ones(tmp_path):
    v = _vault(tmp_path)
    m = eval_score.score_task({"references": ["a"]}, {"self_score": 1.0},
                              eval_score.catalog_citekeys(v), set())
    assert m == {}
...
def test_extract_results_newest_block_wins_and_reads_all_summary_fields():
    card = {
        "task_id": "c1",
        "runs": [{"summary": "```json\n" + json.dumps(
            {"vault_eval": "result", "task": "find-x", "quarter": "2026-Q2",
             "retrieved": ["first"]}) + "\n```"}],
        "metadata": json.dumps({"summary": "```json\n" + json.dumps(
            {"vault_eval": "result", "task": "find-x", "quarter": "2026-Q2",
             "retrieved": ["second"]}) + "\n```"}),
        "summary": "```json\n" + json.dumps(
            {"vault_eval": "result", "task": "verify-y", "quarter": "2026-Q2",
...
def test_score_run_per_task_statuses_and_aggregate(tmp_path):
    v = _vault(tmp_path)
    cards = [
        _result_card("find-x", "2026-Q2",
                     retrieved=["vaswani2017attention"],
                     cited=["vaswani2017attention"], claims=[], self_score=1.0),
    ...
    assert rows["find-x"]["metrics"] == {"recall_at_k": 1.0, "support_rate": 1.0,
                                         "fama_clean": 1.0}
    ...
    assert (agg["tasks_total"], agg["tasks_scored"],
            agg["tasks_reported"], agg["tasks_unscored"]) == (2, 1, 1, 0)
...
def test_append_run_is_one_jsonl_line_per_run(tmp_path):
    ...
    run = eval_score.score_run(
        v, [_result_card("find-x", "2026-Q2", retrieved=["vaswani2017attention"])],
        "2026-Q2")
...
def test_cli_skips_the_log_when_no_results_exist(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr(sys, "argv",
                        ["eval_score.py", "--vault", str(v), "--quarter", "2026-Q2",
                         "--from-json", str(cards_file)])
    eval_score.main()
...
def test_cli_scores_and_appends_from_json(tmp_path, monkeypatch, capsys):
    ...
    cards_file.write_text(json.dumps([
        _result_card("find-x", "2026-Q2", retrieved=["vaswani2017attention"])]),
        encoding="utf-8")
    monkeypatch.setattr(sys, "argv",
                        ["eval_score.py", "--vault", str(v), "--quarter", "2026-Q2",
                         "--from-json", str(cards_file)])
    ...
    line = json.loads((v / "system/metrics/eval/runs.jsonl")
