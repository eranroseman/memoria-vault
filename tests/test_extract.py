...
def test_extract():
    def _run():
        good = (
            "This is a perfectly ordinary paragraph of English prose describing a "
            "study, its methods, and the results obtained over a long evaluation. "
        ) * 6
        garbled = "�� ab � cd � " * 80
        tiny = "short."
        checks = [
            ...
            (
                "tiny -> not ok (too-few-chars)",
                coherence(tiny, pages=1)["reason"] == "too-few-chars",
            ),
            ("empty -> not ok (empty)", coherence("", pages=1)["reason"] == "empty"),
            (
                "no source & no pdf -> degraded",
                extract({}, None)["degraded"] is True and extract({}, None)["source"] == "none",
            ),
...
def test_unpaywall_is_first_and_falls_back(monkeypatch):
    calls = []
    good = (
        "This open access PDF has a clean text layer and enough ordinary "
        "English prose to pass the deterministic coherence gate. "
    ) * 6
    ...
    monkeypatch.setattr(
        _m, "from_pmc", lambda pmcid, email="", api_key="": calls.append("pmc") or "SHOULD NOT RUN"
    )
    ...
    calls.clear()
    pmc_text = (
        "PMC full text is coherent and remains the fallback after an unusable Unpaywall PDF. "
    ) * 7
    monkeypatch.setattr(
        _m,
        "from_unpaywall",
        lambda ids, email="": (
            calls.append("unpaywall") or ("tiny", "pymupdf4llm", "https://oa/bad")
        ),
    )
    monkeypatch.setattr(
        _m, "from_pmc", lambda pmcid, email="", api_key="": calls.append("pmc") or pmc_text
    )
    got = _m.extract({"doi": "10.1/x", "pmcid": "PMC1"}, None, "pi@example.test")
