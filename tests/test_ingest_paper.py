...
@pytest.mark.parametrize(("citekey", "note_type", "item_type", "want"), _EXPECT)
def test_ingest_paper_sets_tier0_identity(
    frontmatter_by_citekey, citekey, note_type, item_type, want
):
    ...
    assert fm["source_id"] == citekey
    assert fm["item_type"] == item_type
    assert len(fm["authors"]) == want["authors"]


@pytest.mark.parametrize(("citekey", "_note_type", "_item_type", "want"), _EXPECT)
def test_ingest_paper_preserves_expected_external_ids(
    frontmatter_by_citekey, citekey, _note_type, _item_type, want
):
    fm = frontmatter_by_citekey[citekey]
...
def test_ingest_paper_frontmatter_order_surfaces_state():
    ...
    assert keys[:5] == ["title", "description", "type", "check_status", "source_id"]
    assert keys.index("citekey") < keys.index("authors")
    assert note["path"] == f"catalog/sources/{citekey}/source.md"
