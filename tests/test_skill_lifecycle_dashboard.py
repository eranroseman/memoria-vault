"""The skill-lifecycle dashboard stays in sync with the files it renders (ADR-43).
...
DASHBOARD = SRC / "system" / "dashboards" / "skill-lifecycle.md"
...
def test_dashboard_ships_and_reads_the_runtime_layer():
    assert DASHBOARD.is_file(), "skill-lifecycle.md missing from src/system/dashboards/"
    ...
    for f in files:
        ...
        for section in ("allow", "deny"):
            ...
            assert got == expected, (
                ...
                "drifted from what skill-lifecycle.md assumes")
