...
- **Do the correct thing, not the cheap thing.** When the right action costs more work than a shortcut, take the right action — and never ask permission to do the obviously-correct thing solely because it is more work. From real failures: delete dead content and fix its inbound links, never leave a stub to dodge the link surgery; read the source of truth (`git show origin/main:<path>`), never the stale copy already open in a worktree; rename a file to its correct name and fix every link, don't defer it as a "want me to?" question; finish the harder half of a task rather than handing it back. The shortcut that trades correctness for less effort is the lazy-wrong path. *(judgment - no mechanism)*
