# `fleet-observability.md` — operational health

**Location.** `00-meta/01-dashboards/fleet-observability.md`

**Status.** Stub — not yet implemented. The runtime queries depend on the scheduled metrics aggregator writing `00-meta/08-metrics/lane-metric-*`; until that is in place this dashboard has no data. The full design lives in the design repo: [memoria-docs/dashboards/fleet-observability.md](../../../memoria-docs/dashboards/fleet-observability.md).
...
- **Not [`drift-watch`](drift-watch.md).** Drift-watch is *structural* health (schema / link / config drift); fleet-observability is *operational* health (cost, retries, trust). Complementary views.
- **Not the Linter verdict.** The lint verdict is a *structural* aggregate; the trust score is an *operational* one.

## Related

- [`index.md`](index.md) — surfaces the per-lane trust-score summary and links here for the contributing inputs.
- [`drift-watch.md`](drift-watch.md) — complementary structural-health view.
- Design spec: [memoria-docs/dashboards/fleet-observability.md](../../../memoria-docs/dashboards/fleet-observability.md).
