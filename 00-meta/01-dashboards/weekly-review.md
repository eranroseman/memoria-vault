...
Include or exclude candidates from the corpus. Requires the candidate frontmatter format from [ADR-21](../../../memoria-docs/decisions/21-shared-candidate-frontmatter.md) — until that decision is adopted and a `candidate-note` template is added, this query returns no results.
