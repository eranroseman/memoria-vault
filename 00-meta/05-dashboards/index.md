...
**Location.** `00-meta/05-dashboards/index.md`
...
**What this is not.** The vault-state queries (unreviewed synthesis, triage debt, evergreen promotion queue, orphan notes) live in [`weekly-dashboard.md`](weekly-dashboard.md) — those belong in the Friday ritual, not the daily glance. This dashboard is the **system-health** view: board queues, drift signals, lane health, cron status. Four sections, each one a one-decision query.

## 1. Today's queue

Cards demanding your attention — `blocked-on-human` for explicit operator decisions, `awaiting-review` for agent-completed work awaiting approval. Oldest first; cap at ten so the dashboard doesn't grow unbounded.

```dataviewjs
const cards = await dv.io.load("00-meta/04-logs/board-state.jsonl");
...
Findings from the Linter's eight structural detectors (M1–M8) in the last 24 hours, severity HIGH or CRITICAL only. Anything that lands here pauses scheduled work — that's what the verdict band's `FAIL` state means. See [`drift-watch.md`](drift-watch.md) for the full M-series view; this section is just the alarm.

```dataviewjs
const text = await dv.io.load("00-meta/04-logs/lint-findings.jsonl");
...
Last run and next run for the four standard cron tasks (see [`07-roadmap.md` standard cron tasks](../07-roadmap/standard-cron-tasks.md)). A missed nightly run is a signal — the system didn't sweep, drift may have accumulated, scheduled cards aren't in the queue.

```dataviewjs
const text = await dv.io.load("00-meta/04-logs/cron-history.jsonl");
const events = text.trim().split("\n").map(l => JSON.parse(l));
const byTask = {};
...
All four standard tasks should appear: `nightly-hygiene` (Linter), `weekly-cluster-report` (Cartography), `weekly-drift-report` (Linter), `fleeting-staleness-report` (Linter). A missing row means cron isn't enabled for that lane (see the [`cron_mode` migration](../07-roadmap/standard-cron-tasks.md#cron_mode-migration)); a row with ✗ in Result means the last run failed.
...
- `00-meta/04-logs/board-state.jsonl` — written by the Kanban dispatcher when cards transition states.
- `00-meta/04-logs/lint-findings.jsonl` — written by the Linter on each pass.
...
- `00-meta/04-logs/cron-history.jsonl` — written by Hermes after each cron task completes.

## What this dashboard does not do

- **Not a vault audit.** Folder counts, orphan notes, stale literature — all live in [`weekly-dashboard.md`](weekly-dashboard.md). The Friday ritual catches those.
...
- **Not a substitute for [`drift-watch.md`](drift-watch.md) or [`audit-log.md`](audit-log.md).** Section 2 (Drift signals) is the alarm; the other two dashboards are the diagnosis.

## Related

- [`weekly-dashboard.md`](weekly-dashboard.md) — Friday-ritual vault-state view (top-to-bottom in 90 minutes).
- [`drift-watch.md`](drift-watch.md) — full M1–M8 detector view + verdict band.
