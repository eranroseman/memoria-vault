...
**Graded loudness decides what reaches it.** Today, `alert`-level findings surface in the above-fold glance; quieter findings stay in the relevant dashboard or are logged only. Push and block routing for louder findings is deferred. The four-level loudness model and the "does it change what the PI does in the next 30 minutes?" test it turns on are owned by [Interaction channels](../../architecture/human-channels.md).
