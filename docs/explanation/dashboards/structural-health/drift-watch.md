...
Loudness is the headline: `alert`-level findings also appear in the Inbox's "Needs me" queue (the Desk workspace). `block`-level push/acknowledgement behavior is deferred; no shipped action stops on loudness alone yet. Everything below alert waits here and in the weekly review. (The four-level loudness model and the "next 30 minutes" test it follows are owned by [Interaction channels](../../architecture/human-channels.md).)
