...
- **Vault audit memory is append-only.** Profiles read it; only the Linter writes to it. `00-meta/02-logs/audit.jsonl` is part of it today; `00-meta/08-metrics/` joins when [fleet observability](../../project/roadmap/future-directions.md#fleet-observability) ships (metrics aggregator is required by Memoria v0.1, not yet built — Phase 6).
