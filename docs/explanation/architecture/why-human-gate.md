...
Agents (Verifier, Linter) can attach a recommendation — `metadata.agent_recommendation` — to a finished card. This recommendation is deliberately separate from the review decision:

- **The agent can be wrong.** A clean Verifier report doesn't mean the draft is good; it means the citations trace and the schema is valid. The human judgment about whether the synthesis is correct, useful, and worth keeping is separate.
- **They can disagree.** A Verifier reports clean; the human reads the draft and rejects it. The separation makes this case coherent — there's no confusion about which verdict counts.
