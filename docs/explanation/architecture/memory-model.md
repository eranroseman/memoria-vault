...
**Audit memory** is append-only because its value is the complete, unmodified record. Agents read it; the Policy MCP writes an entry at every gated write. The constraint is enforced, not advisory — every write is hash-paired (`before_hash`/`after_hash`), and the Linter's `audit-unpaired-writes` detector catches a write whose pairing never completed. Capture must start from day one, because the cost and human-loop trends it tracks can't be reconstructed retroactively.
