...
- **Per-session log files, not a single `log.md`.** The designed narrative session log writes one file per session to `system/logs/sessions/`. With one append-only file, distributed writes from VPS and desktop produce sync conflicts; one-file-per-session has nothing to conflict on. *(Deferred — this log has no writer in 0.1.0-alpha.1; see [ADR-25](../../adr/25-session-logging-two-logs.md). The shipped `audit.jsonl` is a single append-only file, so cross-machine sync of the audit log waits on the multi-machine patterns below.)*
