...
[Karpathy](../../../reference/evidence-and-integrations/bibliography.md#karpathy-llm-wiki)'s Autoresearch pattern — an agent that runs overnight, modifies code, tests against a fixed evaluator, and keeps or reverts — works because three conditions hold simultaneously:
...
**Synthesis quality is not scalar.** Autonomous research systems such as AI
Scientist, [AI co-scientist](../../../reference/evidence-and-integrations/bibliography.md#gottweis2025aicoscientist),
CORAL, [SciMON](../../../reference/evidence-and-integrations/bibliography.md#wang2024scimon), Karpathy
Autoresearch, and [Chen et al. 2026](../../../reference/evidence-and-integrations/bibliography.md#chen2026autonomous)
use one keep/revert number. That can work for their target tasks. It does not
measure whether synthesis is faithful, well-cited, and non-redundant. Citation
count, relevance, and Scite support can rank triage; they cannot prove synthesis
correctness.

**Synthesis errors compound.** In ML benchmarking, a wrong experiment wastes a
run. In knowledge work, a wrong claim stays in the vault and gets cited by later
notes. Errors do not stay local; they accumulate.

**Later sources reinterpret earlier ones.** ML experiments can be independent.
Knowledge work is not. A later paper can overturn an old claim or supply the
framework needed to understand it. A source's value is often visible only in
retrospect.
...
Confidence routing, such as SmartPause in AutoResearchClaw ([Liu et al. 2026](../../../reference/evidence-and-integrations/bibliography.md#liu2026autoresearchclaw)), routes to the human only when self-assessed confidence is low. Memoria refuses it for two reasons:
...
PI approval. A bounded code-experiment loop can be reconsidered only after the
external-coding-agent path is defined beyond the current project handoff. That
requires a superseding decision, not incremental relaxation.
...
[Chen 2026](../../../reference/evidence-and-integrations/bibliography.md#chen2026copilots)'s (*From Copilots to Colleagues*) taxonomy:
L1 autocomplete, L2 multi-step with human approval per step, L3 multi-step with
human-set strategy and per-batch review, L4 self-directed within a domain, and
...
- Borrow/adapt/ignore table: [Pattern provenance table](../../../reference/evidence-and-integrations/pattern-provenance.md)
