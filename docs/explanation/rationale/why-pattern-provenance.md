...
| **Persistent knowledge graph** | [OmegaWiki](../../reference/bibliography.md#qian2026omegawiki), Idea2Story | Preserves relationships instead of re-searching at every query. The graph accumulates; each new source adds to an existing network rather than sitting in isolation. | Wikilinks, typed relations (`supports` / `contradicts`), hubs, entity notes. |
| **Reviewable organization artifacts** | LitLLM, [LatteReview](../../reference/bibliography.md#rouzrokh2026lattereview) | Synthesis is inspectable, not hidden in prompts. The human can read and validate the organization layer independently of the content it organizes. | Canvases, hubs, index notes. |
| **Persistent Kanban + worker lanes** | Hermes Agent (Nous Research) | A durable state machine across sessions and retries — the runtime Memoria builds on; see [Why Hermes](why-hermes.md). | Board layer; the background lanes claim cards; see [The board as a state machine (the control plane)](../workflows/board-as-state-machine.md). |
...
| **File-as-Bus / thin control over thick state** | [Chen et al. 2026](../../reference/bibliography.md#chen2026autonomous) (*long-horizon engineering*) | Independent empirical validation that the durable-artifact bus is load-bearing, not overhead (the ablation numbers are in [Why the architecture is layered](why-three-layers.md)). | The entire layered architecture; see [Why the architecture is layered](why-three-layers.md). |
...
| **Cross-run knowledge accumulation** | [PARNESS (Wang and Luan 2026)](../../reference/bibliography.md#wang2026parness) | Third independent corroboration of the durable-state thesis (the PARNESS finding is quoted in [Why the architecture is layered](why-three-layers.md)). | Vault layer; see [Why the architecture is layered](why-three-layers.md). |
...
| **Agent-readable shared synthesis pool** | [AgentRxiv (Schmidgall and Moor 2025)](../../reference/bibliography.md#schmidgall2025agentrxiv) | Empirical: agents reading prior agent-generated reports outperform isolated agents (the benchmark figure is in [Why the architecture is layered](why-three-layers.md)), validating the Karpathy LLM-Wiki claim quantitatively. | Deferred — adopted in principle; cross-project reading requires enough corpus overlap across projects to be useful, which develops after the vault has been active for some months. |
...
| **Autonomous keep / revert without review** | Karpathy Autoresearch | The three preconditions that make autonomous loops safe for ML benchmarking all fail for knowledge work. See [Why Memoria doesn't pursue full autonomy](why-not-autonomous.md). |
...
| **Confidence-routed bypass of the human gate** | AutoResearchClaw SmartPause (Liu et al. 2026) | SmartPause routes to the human only when the agent's self-assessed confidence is low — converting a structural gate into a probabilistic one that a confidently-wrong agent sails through. Refused; the full argument is in [Why Memoria doesn't pursue full autonomy](why-not-autonomous.md). |
