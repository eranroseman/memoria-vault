...
**The Compile flow** brings new knowledge into the vault — sources become your own integrated claims. `find` is the Librarian surfacing candidate sources; `capture` lands a chosen source as a `paper` entity from the local `.bib` alone — the guaranteed, offline *nothing-lost* floor (`lifecycle: captured`); `enrich` adds network metadata, full text, and ID-keyed entity/citation links and proposes a classification (`captured → proposed`); `classify` is the human accepting the classification and making the note canonical (`→ current`); `discuss` is the reflective phase — questioning the source through the co-PI before any claim is written; `distill` is the human writing a claim note from the source; `connect` is the human relating that claim into the existing graph (typed `links:` — `supports` / `contradicts`). The two human gates are **classify** and **distill** — agents find, capture, and propose; the human canonizes. (`promote` (claim maturity) and `archive` are knowledge-maintenance, not intake phases; a formal systematic review prepends an opt-in `screen` phase.)
