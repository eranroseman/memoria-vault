...
Memoria's knowledge cycle has two flows, each a sequence of **phases**, each with its own direction.
...
```text
assess → frame → sketch → draft → verify → export
```
