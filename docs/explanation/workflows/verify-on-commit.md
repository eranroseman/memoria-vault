...
> **Not built in v0.1.0-alpha.2 — deferred.** The post-commit trigger described here does not exist yet; committing a draft creates no verify card. Use `Memoria: verify draft` from the palette (or ask the Co-PI) to verify manually. Tracked in [#377](https://github.com/eranroseman/memoria-vault/issues/377). The page stands as design intent for the trigger.
