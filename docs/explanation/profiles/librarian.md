...
**Highest external surface in the system — but no longer reached directly.** Librarian is the profile that touches the most external data (OpenAlex, Semantic Scholar, Crossref, PubMed, Unpaywall), but its `web` toolset is disabled: every external lookup goes through a gated MCP server — `paper_search` for discovery, the `ingest` pipeline for server-side enrichment, the read-only `pyzotero` MCP for Zotero. Concentrating the external surface in one profile *and* routing it entirely through audited MCP makes it visible, auditable, and controllable by construction ([ADR-32](../../../project/adr/32-external-access-over-mcp.md)).
