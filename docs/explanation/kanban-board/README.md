...
**Lanes are the four background agents** — Librarian, Writer, Peer-reviewer, Engineer (`assignee = memoria-<name>`). The Co-PI has no lane (it converses at the desk and delegates), and neither do the deterministic engines (ingest, search, clustering, sweeps, the Linter run on cron/CI) — why both stay off the board is in [Board states and the review gate](states.md).
