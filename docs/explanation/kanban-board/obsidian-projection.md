...
The same pass appends a compact snapshot line to `system/logs/board-state.jsonl` with per-lane counts and review-queue depth; the Inbox space reads this instead of re-querying the database, keeping its daily glance lightweight.
