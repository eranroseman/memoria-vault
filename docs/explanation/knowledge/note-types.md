...
The vault distinguishes fifteen note types. But the types are not arbitrary — they map onto three fundamental epistemic roles that define what a note *is* in terms of who created it, from whose perspective, and what status it has in the knowledge system.
...
- **`candidate-note`** — a paper or source the Librarian found during discovery. It's in the screening queue, not yet classified. The human reviews candidates against their screening criteria.

Other working note types — `draft`, `canvas`, `code-note`, `project-note`, `deliverable`, `session-log`, `verification-report` — are artifacts of specific workflow stages. They track active work rather than representing settled knowledge.
