...
(A `candidate-note` — a transient lead the Librarian surfaces during discovery, a Verifier gap-card, or an ingestion dead-letter, all awaiting screening — is the **16th** type, adopted in v0.1 (proposal-21). Session logs and verification reports are workflow *artifacts*, not note types.)
