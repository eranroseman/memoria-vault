...
For the exact field definitions, allowed values, and the open `study_design` vocabulary, see [reference/frontmatter.md](../../reference/frontmatter.md).
...
This is the class of failure the Linter's `schema-check` command is designed to catch, but only after the canonical vocabulary is defined. Before the canonical list exists, drift is entirely silent. See [common-pitfalls.md](common-pitfalls.md) for the concrete failure scenario and how it compounds over time.

## Related

- The Linter's schema-check command: [profiles/linter.md](../profiles/linter.md)
...
- Field definitions and the open `study_design` vocabulary: [reference/frontmatter.md](../../reference/frontmatter.md)
