...
`project` documents under schema-owned Concept homes. Digests and generated
...
A hub is not an index. The failure mode of a hub-as-index is that it becomes a
flat list no one opens because a CLI/read-API query or optional editor view
already lists candidates faster. What a hub adds over a query is perspective: a
framing of what a cluster is about, a curation of what matters most in it, and a
diagnosis of where the cluster needs work.
