...
Canonical zones (*v0.1.1: `notes/claims/` and `notes/hubs/`, per [ADR-47](47-type-first-category-folders.md); the deliverable zone returns with the v0.1.2 Project workspace. Originally `30-synthesis/01-claims/`, `30-synthesis/02-reference/`, `30-synthesis/03-moc/`, `50-deliverables/`*) must only receive content that a human has reviewed and approved. The question is whether this guarantee lives in prompt instructions, agent conventions, or infrastructure.
