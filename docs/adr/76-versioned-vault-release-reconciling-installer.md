...
- **Support burden:** the `sys.path`/`__file__` bootstrapping (11 sites) or the three-way `requirements*.txt` split becomes a recurring source of import/CI breakage — the package + editable-install half (step 2) is the relief and can land first.
