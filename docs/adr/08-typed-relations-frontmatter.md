...
- **Workflows affected:** [Link checked notes](../how-to-guides/knowledge/link-related-claims.md) (where `contradicts` / `supports` are set), [Query](../how-to-guides/knowledge/query-the-vault.md) (relation-aware retrieval).
- **Files affected:** [Frontmatter fields](../reference/frontmatter.md) (the `relations:` namespace + vocabulary), [Document types](../reference/document-types.md) + `99-system/templates/claim-note.md`, the Linter's `structural-detectors.md` (vocabulary check), [contradictions dashboard](../explanation/dashboards/synthesis-agenda.md#contradictions) (consumer).
