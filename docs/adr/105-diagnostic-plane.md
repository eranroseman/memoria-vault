...
- The shared envelope ([ADR-127](127-quarantine-and-verify-integrity.md)) must keep the diagnostic plane's rotation/disposal rules from ever touching the audit plane's append-only-forever rule, and vice versa — the planes share an envelope, not a retention policy.
...
- **Related decisions / Depends on:** [ADR-127 (quarantine-and-verify integrity)](127-quarantine-and-verify-integrity.md) (the plane this details); [ADR-125 (standalone CLI + engine)](125-standalone-cli-engine-architecture.md) and [ADR-125 (standalone CLI + engine)](125-standalone-cli-engine-architecture.md) (why agent cognition is out of scope); [ADR-24 (single-researcher scope)](24-single-researcher-scope.md) (the one-user foot-gun argument).
- **Tracking issue:** [#736](https://github.com/eranroseman/memoria-vault/issues/736) — implementation-readiness tracker; closed after `src/memoria_vault/runtime/diagnostics.py` and its unit tests shipped.
