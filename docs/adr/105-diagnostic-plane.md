...
- **Outside the vault, outside Git.** Diagnostic files live under an OS state directory (e.g. `$XDG_STATE_HOME/memoria/` or `~/.local/state/memoria/`), **never** under `system/logs/` or anywhere in the Git-tracked, sync-tracked vault tree. A `.gitignore` entry is a belt-and-suspenders backstop, not the primary control. Each machine keeps its own; diagnostics are not shared state.
