...
> *Terminology note (0.1.0-alpha.2): "MOC" is now the `hub` type ([ADR-50](50-universal-lifecycle-and-maturity.md)); "the Mapper" is the **Librarian's `map` lane** ([ADR-48](48-copi-and-agent-consolidation.md)); the `reference` type referenced below is retired ([ADR-50](50-universal-lifecycle-and-maturity.md)) — an `evergreen` claim is the settled unit. The threshold-alert decision is unchanged.*
