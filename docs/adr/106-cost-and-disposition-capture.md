...
Because this couples to Hermes's internal, undocumented SQLite schema, the **Hermes version is pinned** and the join is re-verified on each Hermes upgrade. A local model-gateway proxy at the profile `base_url` remains a documented future option if provider-independent or actual (not estimated) cost is ever required, but is **not adopted** now: it would re-capture data Hermes already stores and add a process in the inference hot path.
...
- Coupling to a private Hermes DB schema is brittle across upgrades — pinning plus an upgrade-time re-verification is the standing maintenance cost this accepts.
