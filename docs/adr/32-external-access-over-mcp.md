...
- **Files affected:** `vault/.memoria/engines/sweeps/retraction.py` (was planned as `mcp/verify_mcp.py` — see the alpha.2 note above), `vault/.memoria/scripts/retraction-refresh-cron.sh`, every `vault/.memoria/profiles/memoria-*/config.yaml` and `vault/.memoria/lane-overrides/*.yaml`, the Linter's `structural-detectors` skill, the Verifier's `retraction-check` + `claim-checks` skills.
