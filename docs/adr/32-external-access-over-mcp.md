...
- **Cost.** More MCP servers to install and keep running. Two require a one-time host install (`pip install paper-search-mcp`, `pip install "pyzotero[mcp]"`); the self-hosted servers (`policy`, `ingest`) run from the vault venv. The Retraction Watch CSV needs a periodic cron refresh through the retraction Operations engine.
