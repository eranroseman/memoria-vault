...
- **Related decisions / Depends on:** [ADR-120](120-profile-config-materialization.md) (profile config materialization), [ADR-28](28-write-gate-as-plugin.md) (the policy gate this rule rides on), [ADR-30](30-deterministic-ingest-pipeline.md) (the pipeline-as-MCP precedent), [ADR-31](31-native-obsidian-mcp.md) (the obsidian MCP).
- **Files affected:** `memoria_vault.runtime.subsystems.integrity.retraction.retraction` (was planned as `mcp/verify_mcp.py` — see the alpha.2 note above), `.memoria/scripts/cron-runner.sh`, and the Linter's structural detectors.
