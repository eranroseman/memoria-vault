...
- **Fleet observability aggregator.** A scheduled aggregator reads the audit log and board history to materialize the [fleet-health dashboard](../explanation/dashboards/operational-health.md#fleet-health) — per-lane and per-skill cost, success rate, retry rate, and latency on daily/weekly/monthly cadence. This slice is built as `metrics_aggregate.py` and the `memoria-metrics` cron wrapper.
