...
| `actor` | string | Policy-gate actor identity named by optional adapter policy (e.g. `"adapter"`) — a free-form name, not the fixed operation-request actor vocabulary (`pi \| agent \| operation \| integrity`) documented in [Control plane reference](control-plane.md#actor-authority-guard). |
...
- Audit-memory substrate: [Memory substrates](../pipelines-and-io/memory-substrates.md)
