...
| 9. Secrets + next steps | Prints where keys go (`~/.hermes/.env` -> re-run `--profiles-only` to propagate) and the first-session checklist (use the space nav row to open Library, then open the Agent Client pane when you want conversational help). |
