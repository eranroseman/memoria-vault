...
| Analyze gaps | worker operation `analyze-gaps` + runtime helper (`analyze_gaps`) | Counts checked-current source/digest/accepted-note topic signals and reports `new-topic`, `undigested`, and `under-warranted` gaps with proposed seed actions; when a project path is supplied, it also seeds project scope/facet terms, counts checked linked thesis terms, and includes checked project argument-health gaps. Repeated off-vocabulary phrases in checked Work text surface as unchecked tag-candidate attention; the operation never writes tags directly. |
...
| Check source metadata | runtime integrity helper (`check_source_metadata`) | Flags checked `source` Concepts that lack citekey, CSL-JSON basics, issued year, an external resource/identifier, carry conflicting DOI metadata, share an exact source external ID with another catalog row, hit the deterministic title/year/first-author duplicate block, or expose duplicate person ORCID/OpenAlex IDs; active record-linkage findings also emit stable `inbox/` work prompts for PI review without merging records. |
