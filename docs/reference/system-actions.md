...
| Record empirical event | worker operation `empirical-event-record` + runtime helper (`record_empirical_event`) | Validates one strict `empirical_event.v1` payload, requires `idempotency_key=empirical-event:<event_id>`, rejects raw text/path-like fields, and appends a queryable `empirical-event` journal row with `journal_event_ref.v1` output metadata. |
