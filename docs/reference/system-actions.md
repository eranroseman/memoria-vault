...
| Analyze project | `memoria project gaps <project-path>`, `trace`, `export` | Runs checked graph and project-argument gap analysis, argument tracing, and deterministic Markdown/Pandoc project export from the CLI control plane. Project Concepts are authored as checked Markdown under `knowledge/projects/` and observed by `memoria workspace scan`. |
