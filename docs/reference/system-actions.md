...
| Analyze project | `memoria project gaps <project-path>`, `trace`, `export` | Runs checked graph, project-scope, linked-thesis, and project-argument gap analysis, argument tracing, and deterministic Markdown/Pandoc project export from the CLI control plane. New Project Concepts are authored through `memoria new project`, queued as `create-concept`, and remain unchecked until review/check promotion. |
