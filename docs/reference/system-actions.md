...
| Capture PDF source | worker operation `capture-pdf-source` + runtime helper (`capture_pdf_source`) | Uses the optional PyMuPDF parser to extract page text from raw PDF bytes, derives page/text/bbox annotation refs for requested quotes, and writes the same checked source Concept path. |
