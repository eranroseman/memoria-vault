...
| Check source metadata | runtime integrity helper (`check_source_metadata`) | Flags checked `source` Concepts that lack citekey, CSL-JSON basics, issued year, an external resource/identifier, carry conflicting DOI metadata, share an exact source external ID with another catalog row, or hit the deterministic title/year/first-author duplicate block; active record-linkage findings also emit one stable `inbox/` work prompt for PI review without merging records. |
