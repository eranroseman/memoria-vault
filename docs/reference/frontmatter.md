...
**Partially generated.** The lifecycle, enum, and per-type field tables are
regenerated by `python scripts/gen_reference_refs.py --write`; edit the schema
sources, not the generated tables.
...
Each type's schema declares the **subset** it uses (validated as `enum:lifecycle`;
the subset must be within the universal chain — test-enforced). The generated
schema inventory above is the exact current subset map.
