...
| --- | --- |
| full chain | `source` |
| `proposed → current → archived` | `candidate`, `gap`, `flag`, `alert`, `pattern` |
...
Inbox cards split into two shapes ([ADR-51](../adr/51-inbox-category-and-honesty-card.md)):
...
`agent_recommendation` never appears on a proposal — that separation (no verdict on proposals) is enforced by the shared card writer [src/.memoria/engines/lib/inbox.py](../../src/.memoria/engines/lib/inbox.py).
