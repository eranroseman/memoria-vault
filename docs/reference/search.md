...
| `memoria project gaps <project-path>` | Gap discovery over checked Work text, graph neighborhoods, SQLite source/topic evidence, and checked project argument health; SQLite-backed source gaps can propose unchecked candidate Works from first-order reference/related edges even when qmd is unavailable. |
