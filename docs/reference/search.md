...
| No standalone duplicate sweep | Exact source external-ID collisions and deterministic title/year/first-author duplicate blocks are flagged by `check-source-metadata`; broader retrospective duplicate detection remains operation/sub-check work. | The shipped search surface is retrieval, not dedupe. |
