...
**Implementation note.** Items marked `# pending` are designed but not yet present in the v0.1 starter vault. Items marked `# deferred` are planned for a future version. See the [v0.1 release plan](../../project/releases/v0.1/release-plan-v0.1.md) for build state.
...
│   ├── memoria-librarian/
│   │   ├── SOUL.md                 # shipped
│   │   ├── config.yaml             # shipped (model routing + mcp_servers + policy-gate plugin)
...
│   └── memoria-linter/
│       ├── SOUL.md
│       ├── detectors.py            # shipped (9 functions: 3 structural detectors + 6 housekeeping checks)
