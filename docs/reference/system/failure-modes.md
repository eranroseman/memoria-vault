...
`loudness: block` prompts affect only the optional policy-hook path. Alert and
block cards remain pull-only Inbox projections. The standalone CLI/worker path
is not paused by loudness.
