...
| Filtered views returning nothing | HIGH | `methodology` or `topics` field value doesn't match the controlled vocabulary exactly — looks like "nothing to do" | Check values in notes match [Vocabulary](../data-model/vocabulary.md) exactly — full recipe: [Fix missing query results](../../how-to-guides/troubleshooting/fix-missing-query-results.md). |
...
| Schema mismatch in filtered views | MEDIUM | A hand-authored note or stale sandbox fixture does not match the current schema | Repair the specific note or reinitialize the sandbox from the current package seed, then validate with `python3 -m memoria_vault.runtime.subsystems.integrity.linter.detectors --vault .`. |
