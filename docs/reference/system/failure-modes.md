...
| Backup target is absent after interruption | HIGH | The process stopped while replacing an existing backup target | Run `memoria workspace recover`; it restores the target recorded by the live backup-transaction marker or retains a completed validated replacement. |
