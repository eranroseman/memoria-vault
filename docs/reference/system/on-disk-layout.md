...
| `.obsidian/plugins/memoria-obsidian/` | Generated release artifacts installed by `memoria init`: `main.js`, `manifest.json`, and `styles.css`. `packages/memoria-obsidian/` owns the authored source and build; the plugin calls local HTTP and records empirical events through `/operation/run`. `memoria init` seeds the artifacts once and never overwrites an existing copy; `memoria doctor --repair` restores them from the package. |
