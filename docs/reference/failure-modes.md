...
| Profile install drift after edit | LOW | Vault source changed but profiles not re-deployed | Re-run `bash install.sh --profiles-only` (`.\install.ps1 -ProfilesOnly` on Windows). |
