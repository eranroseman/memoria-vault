...
| Broken frontmatter YAML | MEDIUM | YAML parse error: unclosed string, list indentation error, missing closing `---` | Open in an editor outside Obsidian (Obsidian masks raw YAML). Fix manually. Verify with `python3 .memoria/operations/integrity/linter/detectors.py --vault .` (the Linter is an engine, not an agent — there is no `memoria-linter` profile). |
