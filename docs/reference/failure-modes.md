...
| Future editor adapter can't connect | MEDIUM | An external adapter added outside the alpha.16 baseline has a stale command path or local app configuration | Use the standalone `memoria` CLI first, then repair the adapter configuration outside the core installer. |
