...
| Cron job didn't fire overnight | MEDIUM | Sleep-prone host or stale `.env` (`always-on` only) | Check `journalctl --user -u hermes-overnight` and the latest `cron-heartbeat.jsonl` row — full recipe: [Set up a VPS](../how-to-guides/setup/set-up-vps.md). |
