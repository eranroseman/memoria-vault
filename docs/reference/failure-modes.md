...
| Optional Obsidian adapter can't connect | MEDIUM | The external adapter command path or local app configuration is stale | Use the standalone `memoria` CLI first, then repair the adapter configuration outside the core installer. |
| `_proposed_classification` not appearing | MEDIUM | The source was captured but enrichment/classification did not run or did not produce a checked result | Run `memoria work enrich --work-id <id>` and inspect the request with `memoria request show`. |
