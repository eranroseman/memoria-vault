...
| Dataview queries returning nothing | HIGH | `study_design` or `topic` field value doesn't match the schema vocabulary exactly — looks like "nothing to do" | Check values in notes match [Frontmatter fields](frontmatter.md) vocabulary exactly. |
