...
| Enrichment block empty after ingest | HIGH | `OPENALEX_API_KEY` missing in per-profile `.env` (silent — ingest "succeeded" with degraded data; OpenAlex requires a key since 2026-02) | Populate the per-profile `.env` ([Set up Hermes](../how-to-guides/setup/set-up-hermes.md)), then re-ingest or rerun the source metadata check. |
...
| Citekey not found at ingest | LOW | `.bib` not updated or not pulled | Refresh the Zotero export and runtime bibliography. |
