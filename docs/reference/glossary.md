...
**Deployment option** — how the vault and execution layer sync across machines. Four patterns: `local-only` (single device, v0.1 default), `local-mesh` (Syncthing, peer-to-peer), `obsidian-sync` (cloud-managed), `always-on` (Syncthing + VPS). See [deployment options](../explanation/deployment/deployment-options.md) for the adopted `local-only` default; the multi-machine patterns are the [multi-machine deployment proposal](../../project-files/proposals/surveys/multi-machine-deployment.md).
