...
| `read_canvas_forks(workspace, project_path, read_scope=None)` | Reads a project's canvas fork status: each `scratch-*.canvas` fork diffed against a live re-render, never the on-disk `argument.canvas`. |
