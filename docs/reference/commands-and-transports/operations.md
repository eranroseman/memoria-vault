...
| Worker requests | `memoria_vault.runtime.worker` | CLI / scheduled tasks | scheduled tasks, CLI, tests, debug sessions | Persists request state in SQLite and executes trusted-write, capture, enrichment, synthesis, integrity, project, projection, attention, eval, and prompt-operation jobs. |
...
| Tracked projections | `memoria_vault.runtime.projections` | SQLite worker request | worker, tests, debug sessions | Regenerates root `index.md` and `bibliography.bib`; the drift check regenerates the tracked set into a temp tree and diffs it against the workspace copies. |
