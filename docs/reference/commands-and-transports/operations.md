...
| Integrity routing and rollback | `memoria_vault.runtime.grounding` | SQLite worker request | worker, tests, debug sessions | Runs metadata, evidence, citation, quote, prompt-injection, provenance, contradiction, and link checks; records verdicts; routes review attention; and handles downstream rollback. |
...
| Integrity retraction | `memoria_vault.runtime.sweeps.retraction.retraction` | None | operator-managed scheduler, CI, PI | Performs read-only DOI lookups; `--sweep` scans checked SQLite Catalog Works and writes Inbox `alert` attention items for retracted hits without changing Work state. |
