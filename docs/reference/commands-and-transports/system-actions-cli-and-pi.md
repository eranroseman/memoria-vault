...
| Refresh projections and search | `memoria workspace rebuild` (add `--search` to also rebuild the search index) | Regenerates tracked projections, bibliography, and workspace indexes from worker-readable state; checked-only search inputs are rebuilt only when `--search` is passed. |
...
| Back up / restore workspace | `memoria workspace backup <target>` / `memoria workspace restore <source> [--force]` | PI-only. Backup copies SQLite, blobs, and the journal head into a manifest-bound directory outside the live vault; restore validates that manifest and requires `--force` while a live database exists. |
| Observe PI edits | `memoria workspace scan` / `memoria serve --watch` | Runs `observe-pi-edits`: it commits direct PI Concept edits with backfilled `derived` events, reconciles baselines for valid bundle Concepts, and returns ask-route `foreign-edit` or `restriction-key-removed` findings when appropriate. `serve --watch` is only a stdlib polling trigger; the scan worker remains the correctness boundary. |
...
| Acknowledge attention | `memoria operation run acknowledge-attention --payload-json '{"target_id": ...}'` | Runs the PI-only `acknowledge-attention` operation: records an `acknowledged` resolution event against the attention target. Unlike `attention resolve`, it has no dedicated CLI subcommand — it is only reachable through the generic operation runner or HTTP/MCP. Unlike `resolve-attention`, it does not emit a `disposition.v1` event and does not update or close the attention projection's frontmatter — the item stays open for a later apply/reject/defer disposition. |
...
| Verify journal | `memoria journal verify` | Runs the one authoritative trust-read path over the event log: checks the hash chain, the live-tip anchor, the committed anchor prefix, and the JSONL export subset. |
...
| Evidence disposition | `memoria project resolve-evidence` records the PI's accept, reject, edit, or defer decision for one draft evidence item. A CLI invocation declared `agent` is rejected. `resolve-evidence` is also a worker operation reserved to actor `pi` in the [Actor Authority Guard](../control-and-policy/control-plane.md#actor-authority-guard), so the loopback [HTTP transport](local-http-transport.md) — which carries PI authority — reaches it through `POST /operation/run`; [MCP](mcp-transport.md), bound to actor `agent`, does not. |
