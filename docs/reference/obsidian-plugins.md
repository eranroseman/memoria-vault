...
| memoria-inspector | `memoria-inspector` | Memoria-authored sidebar pane for board counts, checked Concept graph browsing, recent audit entries, integrity flags, the Linter verdict band, and enqueue-only worker controls for checks, capture, curation, rollback, and attention decisions ([ADR-121](../adr/121-enqueue-only-obsidian-control-panel.md)). |
