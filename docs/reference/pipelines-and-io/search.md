...
## Graph-aware retrieval and clustering boundary

Memoria keeps graph-aware retrieval and gap analysis in the standalone runtime,
but it does not ship a clustering adapter or heavyweight topic-modeling stack.
...
| Text-ranked graph context | Search ranks textual Work, Concept, and graph-neighborhood documents; it does not run graph algorithms. | Relationship-aware retrieval beyond first-order neighborhoods belongs to the graph-aware retrieval boundary above. |
...
- The deterministic methods catalog: [Retrieval and analysis methods](../analysis-and-surfaces/retrieval-and-analysis-methods.md)
