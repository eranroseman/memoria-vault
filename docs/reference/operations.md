...
| Tracked projections | `memoria_vault.runtime.projections` | Worker queue | worker, tests, debug sessions | Regenerates root and bundle `index.md` plus `references.bib`; the drift check regenerates the tracked set into a temp tree and diffs it against the workspace copies. |
