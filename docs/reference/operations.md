...
| Trusted writer | `memoria_vault.runtime.trusted_writer` | Worker queue | worker, tests, debug sessions | Stages Concepts unchecked, promotes only checked Concepts, observes PI edits from git status, quarantines untraced bundle changes from git status or explicit paths, rebuilds trace state, and commits only selected writer paths plus the journal. |
