...
| Metadata check | `check_source_metadata()` / worker `check-source-metadata` | Flags checked sources that lack citekey, CSL-JSON basics, issued year, an external resource/identifier, carry conflicting DOI metadata, share an exact source external ID with another catalog row, or hit the deterministic title/year/first-author duplicate block. Active record-linkage findings emit one stable `inbox/` work prompt for PI review; the operation never merges records. |
