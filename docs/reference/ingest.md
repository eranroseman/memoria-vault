...
| DOI enrichment | `enrich_source()` via worker `enrich-source` | Requires Crossref, OpenAlex, and Unpaywall payloads for DOI records, caches raw provider JSON under `.memoria/blobs/provider-payloads/`, merges canonical CSL-JSON, external IDs, and field provenance, then checks the catalog row and materializes `references.bib`. |
...
| Provider policy | `.memoria/enrichment/providers.yaml` | Declares required DOI providers, endpoint templates, timeouts, and contact-email environment variables. The operation manifest allowlist must agree with enabled provider base URLs. |
...
| SQLite catalog row | `memoria_vault.runtime.state` | Writes staged or checked source metadata, enrichment runs, provider payload paths, external IDs, and field provenance in `.memoria/state/memoria.sqlite`, the alpha.13 catalog working-state source of truth. |
