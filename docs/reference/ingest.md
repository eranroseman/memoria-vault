...
| Catalog source row | catalog state | Source metadata lives in `.memoria/memoria.sqlite` with a mirror concept id of `catalog/sources/<work_id>` and DB/read-API `check_status`. Human interpretation starts in `notes/` as `mode: work` notes. |
