...
| `skeleton-drift` | MEDIUM | A directory from the schema skeleton (the `skeleton` list in `.memoria/schemas/folders.yaml`) missing from the vault — run `memoria doctor --repair` or create it. Checked only in installed vaults (`.git` present); the package seed ships no empty content dirs. |
...
```bash
python3 -m memoria_vault.runtime.subsystems.integrity.linter.detectors --vault <vault> [--json]
...
The installer wires the package-seeded `.githooks/pre-commit` into the deployed
vault's `.git/hooks/pre-commit`. On every commit it passes the staged `.md`
paths to `memoria_vault.runtime.subsystems.integrity.linter.precommit_check`,
which validates each typed document against its schema via the shared loader
(`memoria_vault.runtime.subsystems.lib.schema`). Any error blocks the commit
(exit 1). Exempt: untyped `system/` infrastructure, vault-root nav pages, and
