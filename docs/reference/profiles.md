...
These are the `MEMORIA_ENV=prod` defaults rendered by the installer. Linux/WSL test installs render all five profiles to Kilo Llama 4 Scout with `MEMORIA_ENV=test`; see [Installer environment overlays](installer.md#environment-overlays).
