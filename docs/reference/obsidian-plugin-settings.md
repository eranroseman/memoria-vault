...
| Setting | Required value | Constraint |
| --- | --- | --- |
| `formDefinitions[].name` | `memoria-fleeting-capture`, `memoria-source-capture`, `memoria-claim-capture`, `memoria-project-start` | These forms are generated by `scripts/gen-forms.py` and must remain present in the committed plugin data. |
