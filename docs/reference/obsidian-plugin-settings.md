...
| Setting | Required value | Constraint |
| --- | --- | --- |
| `formDefinitions[].name` | `memoria-fleeting-capture`, `memoria-source-capture`, `memoria-claim-capture`, `memoria-hub-capture`, `memoria-project-start`, `memoria-thesis-capture` | All six forms are generated by `scripts/gen-forms.py` and must remain present in the committed plugin data. `memoria-fleeting-capture`, `memoria-source-capture`, and `memoria-project-start` are active PI-facing flows; the other three are generated schema-aligned capture surfaces with no dedicated palette wrapper yet. |
