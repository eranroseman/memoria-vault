...
| Setting | Required value | Constraint |
| --- | --- | --- |
| `formDefinitions[].name` | `memoria-fleeting-capture`, `memoria-source-capture`, `memoria-claim-capture`, `memoria-project-start` | Generated by `scripts/gen-forms.py` from each form type's `creation.form` block in `vault-template/.memoria/schemas/types/`; `tests/test_modalforms.py` fails if the committed plugin data drifts. |
