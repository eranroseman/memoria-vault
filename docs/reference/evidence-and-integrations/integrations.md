...
| Writes | `POST /operation/run` only; the HTTP transport records actor `pi`. Empirical events use operation `empirical-event-record` and idempotency key `empirical-event:<event_id>`. |
