...
> **Scope vs. the rejection.** This is a shared **operational-memory** layer (cross-machine `MEMORY.md` / `USER.md` recall), *not* a knowledge store. External memory products *as a knowledge layer* are [consciously rejected](hermes-capability-adoption.md#consciously-rejected) — the vault is the knowledge store. The same products (Mem0, Honcho, …) appear here only as an operational-memory transport for concurrent multi-machine use.
