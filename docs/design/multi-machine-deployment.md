...
4. **Local full install** only for a *developer* role (see [Phase 4 install discipline](../../project/release/v0.1/release-plan-v0.1-appendix.md)), never for the principal investigator's laptop in human-as-reader mode. The dev install must use `HERMES_HOME` isolation and point at a *test vault*, never the production vault.
