...
**Not a single-agent system.** "One model does everything" produces an agent with unclear responsibility, ambiguous permission boundaries, and no separation between discovery and synthesis. Memoria explicitly avoids this. Five profiles — one conversational Co-PI plus specialist background profiles, each with narrow permissions and a clear exit condition — replace one generalist.
...
- Why the human gate is structural: [Why the review gate is structural](why-review-gate-is-structural.md)
- Why L3 is the ceiling: [Why Memoria doesn't pursue full autonomy](why-not-autonomous.md)
