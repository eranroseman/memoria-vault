...
If the concept you need isn't in the glossary yet, that's a signal — either you're inventing language the project doesn't use (reconsider) or there's a real gap (note it, and add the ruling to the glossary when it resolves).
...
silently overriding — name the ADR and say why it is worth reopening.
`docs/adr/` holds no ADRs yet, so this bites only once one lands.
