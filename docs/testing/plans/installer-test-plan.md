...
- ✓ Pass: the 13 plugin folders present; secret configs shipped as `data.json.example` (not real `data.json`) for `obsidian-local-rest-api` and `agent-client`.
...
| C | 13 plugins copied; MCP venv wired; CSS snippets enabled | | |
