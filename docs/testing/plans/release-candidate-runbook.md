...
- [ ] **Gate contract — Hermes drift:** `python src/.memoria/mcp/hermes_contract_doctor.py` exits 0 — every direct-capability tool the *installed* Hermes ships (file/terminal/code_execution toolsets) is hard-denied. **Re-run after every Hermes upgrade**; this is what catches a new tool like `process` the denylist doesn't yet know. WARN lines (dead names, the egress allow-by-default surface) are informational, not gate failures.
