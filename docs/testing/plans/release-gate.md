...
This runs Source, Package, and the opt-in Runtime smoke. Complete the installer,
Runtime-on-installed-candidate, Product, failure/recovery, blocker,
published-docs, and cut checks below.
...
| Step | Evidence |
| --- | --- |
| Automated sweep | `scripts/verify rc` passes or records an explicit Runtime skip reason. |
...
| Runtime on installed candidate | [Runtime Gate](runtime-gate.md) passes on the installed candidate. |
...
Record results in the release parent issue or readiness/stage sub-issues. Copy a
curated summary to a release `validation-log.md` only when the issue trail is
insufficient.
