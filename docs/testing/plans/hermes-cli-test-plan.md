...
| L6 | `classify` | a `paper-note` with empty/low-confidence `_proposed_classification` | `classify <citekey>` | `_proposed_classification` re-proposed (values from the controlled vocabulary — [Frontmatter fields](../../reference/frontmatter.md)); human fields still empty |
...
| B12 | review-gate check: `hermes kanban` action advancing a card out of `done` without `review_status: approved` | refused — the gate is a dispatch precondition (see [Policy MCP](../../reference/policy-mcp.md)) |
