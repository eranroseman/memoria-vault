...
**This overlaps [golden-path](e2e-golden-path-plan.md) Part A** but is narrower and deeper: it isolates the ingest lane, adds the Tier-1 **correctness** check the golden path assumes, and ends at the review handoff (the loop *closing* — human approve → `current` — is [G11](../../releasing/v0.1/release-plan-v0.1.md)).
