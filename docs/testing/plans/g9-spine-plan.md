...
**G9 green** when one card traverses A-min → D with the gate logging `allow_with_log`, an `audit.jsonl` row whose `after_hash` matches the saved report, and the card ending `done` — and all E invariants hold. A-cron green adds the scheduler + cron→card proof. Record in [Release plan — v0.1.0-alpha.1](../../releasing/0.1.0/release-plan-0.1.0.md) (gate G9).
