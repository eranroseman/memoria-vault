...
**Concrete.** (a) Model: `MoritzLaurer/DeBERTa-v3-base-mnli-fever-anli`, read `id2label` (label order differs across NLI models — never hardcode). (b) Verdict schema `{verdict, evidence_sentences[], score}`, but **gate REFUTED on variable/context match** before it becomes a contradiction edge. (c) MaxSAT consistency pass over the *gated* edges only — surfaced to the gate, never auto-applied (proposer precision is the least-reliable memory capability — track it first). (d) Keep the `NEUTRAL` (different-variable) fixture as a **regression test**, not just the HANS/trap one — the neutral class is the weak spot, and re-run on real vault claim pairs (`current-state-baseline.md`) before committing.
