...
_Working artifact for the 0.1.0-alpha.9 checkpoint; deleted at close-out (AGENTS.md). Every durable finding is **landed** (PRs #820 / #825 / #830) or **filed** (issues #822–#829) — see §5. This file is the readable synthesis until the close-out sweep removes it._

> **What this is.** It began as a "clean-slate design" for how Memoria uses Hermes — and isn't one. On-box verification refuted nearly every proposed deletion, so it became a **verification audit + a change ledger**. The single genuinely new design claim (§4) is small and only half-built. Honest self-assessment: rigor was **mis-allocated** — the cheap-and-checkable (tool names, deletions, probes) got hard verification; the expensive-and-load-bearing (the §4 attribution claim, the §3 upgrade decision) got anecdote or omission until forced onto the page. The through-line of every finding: **enforcement is a mechanism, not a label** (now an AGENTS.md working principle) — a guarantee kept getting credited to a *description* (a config key, a taxonomy, "the architecture") when a *mechanism* elsewhere was what actually held it.
...
- **Severity is not "low."** On the normal model path it is low (0.14 providers reject out-of-schema calls); on the **injection path — the stated adversary — it is unmitigated and P0**, violating req-4 (bounded autonomy).
...
- **Landed:** `process` fix + completeness sweep + Hermes contract doctor (PR #825). **Open:** default-deny per-lane (#822), registration fail-open (#823).

### 2b. ADR-implementation gaps — all 75 accepted ADRs audited
**4 UNIMPLEMENTED · 1 DIVERGED · 10 PARTIAL · 60 IMPLEMENTED.** The load-bearing one:
...
**Recommendation:** do #822/#823 regardless (needed on either branch); take **(A)** for the platform features — upgrade Memoria-test under an isolated `HERMES_HOME`, re-run the ADR-106 cost-doctor + the new contract-doctor, then graduate or refute each isolated-source item. Filed as **#828**.
...
| O1 | **Gate default-deny per-lane** + ADR-80 deny-test — the egress fail-open (§2a). *Stopgap (process+sweep+doctor) landed in L3.* | 🔲 | #822 | M |
...
| O6 | **Upgrade decision** — 0.17 test vault vs stay on 0.14 (§3) | 🔲 | #828 | decision |
| O7 | **Multi-contested card-attribution** design — the req-8 claim (§4) | 🔲 | #829 | design |
...
**Sequencing:** O1+O2 (the P0 security pair) first, regardless of the upgrade fork; O4 (live correctness bug) next if alpha.9 touches supersession; O6 gates whether the isolated-source items are worth chasing at all. U1–U4 are independent cleanups.

## 6. Deletion-safety

This doc routes to durable homes (§5). Everything is **landed** (PRs #820/#825/#830) or **filed** (#822–#829) **except U1–U4**, which exist only here. So: **safe to delete once U1–U4 are filed** — until then those four cleanups evaporate with the file.
