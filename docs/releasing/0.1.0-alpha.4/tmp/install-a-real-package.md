...
| Code integrity | **Not golden.** Code is protected by the **MCP-only sandbox** (agents cannot write files; [[mcp-only-agent-sandbox]]) + **git** as source of truth. ADR-55 golden covers *vault system files only* — `golden_restore.py` manifest = `system/{templates,dashboards,patterns,eval,scripts}/` + `home.md`/`system/vocabulary.md`/`AGENTS.md` + shipped `.obsidian` config files; **no `.memoria/` prefix** (not code, not `schemas/`, not `lane-overrides/`) | `golden_restore.py:30-50`, [ADR-55](../../../adr/55-src-scaffold-populate-golden-copy.md) |
