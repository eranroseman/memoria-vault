...
> **Superseded/expanded by the companion report** ([open-issues-research.md](open-issues-research.md) Issue 2), which derives the vocabulary from definitions rather than the existing names. Net decision below.

- **[ADR] Rename the layer to "Operations" (delete both "engine" and "app").** Self-descriptive, the clean opposite of *agents*; "engine"/"app" is a one-concept-two-words anti-pattern (DDD ubiquitous language). Gloss: "you *run* an operation, you *delegate* to an agent." ("engine" is an acceptable fallback only if churn is unacceptable — but still delete "app".)
- **[ADR] Four self-descriptive categories — Operations → Processing · Integrity · Cleanup · Telemetry** (replaces the loose "processing/maintenance vs bookkeeping/housekeeping" four-word framing). Each is industry-standard and non-overlapping; mapped by *what the PI does with the output*: Processing = uses (ingest/search/cluster); Integrity = acts on, via cards (verify/lint/golden); Cleanup = never sees (reconcile/archive/retry); Telemetry = consults (board export/metrics/digests/eval). Restructure the tree to `operations/{processing,integrity,cleanup,telemetry}/` — which also rehomes the stranded `cluster_mcp.py` and gives Search a directory. Ties to §3: Integrity → status bar + Action cards; Telemetry → pull dashboards; Cleanup → invisible.
- **[FIX] Define "CI invocation"** on first use, for non-developers: "CI (the automated check that runs when changes are proposed) can invoke operations directly, at the same trust level as cron and the PI — bypassing the MCP boundary that agents must use." Or replace "CI" with "automated checks" if too developer-centric.
- **[ADR] Split ingest docs by Diátaxis.** Create `explanation/.../ingest` (rationale: why the uncertainty floor, why automated-not-gated), keep `reference/ingest` purely procedural, and move the sweeps content out of `reference/ingest` into its own reference. Generalizes: each operation gets a procedural reference page + a rationale explanation page (Django "link, don't repeat"). See the Diátaxis audit (companion §"Diátaxis audit" / [naming-and-diataxis-audit.md](naming-and-diataxis-audit.md)) for the full list of mixed pages.
- **[ADR] Naming consistency — one bare verb per leaf operation:** `ingest`, `search`, `cluster`, `verify`, `lint`, used for name + dir + module + CLI verb. Rename `engines/ingest/pipeline.py` (generic shape-name) to a responsibility name (`runner.py`/`catalog.py`). Structural rename — sequence *after* the source-link convention (§8) so links aren't pinned to paths about to move.
...
- **Broken links to source are architectural, not a depth bug.** The site is **Jekyll + just-the-docs on GitHub Pages** (`docs/_config.yml`, served at `eranroseman.github.io/memoria-vault`). Jekyll's source root is `docs/`; **`src/` is a sibling of `docs/`, outside the site** — so `src/` is never published. Therefore *every* relative link from a docs page into `src/` breaks on the live website, regardless of how many `../` it uses. A "correct-depth" `../../../../src/...` would still 404 on the site (and just-the-docs permalinks change the served URL structure, so relative traversal out of the site root resolves to nothing).
- **The earlier "✓ works" verdicts were filesystem checks, not website checks.** Links like `docs/reference/ingest.md` → `../../src/...` resolve on disk / in GitHub's file browser, but **also break on the published site**. Depth only decides whether a link happens to resolve in the GitHub file viewer — it has no bearing on the website.
- **Caveat on docs-doctor:** if `docs-doctor` resolves links against the filesystem, a `src/` link can *pass lint and still be dead on the website*. Passing the lint job is not proof the site link works.
...
- **[FIX] Stop linking into `src/` from docs — it cannot work on the website.** Because `src/` is outside the Jekyll site, no relative link to source resolves on the live site at any depth. Adopt one convention for referencing a system file:
  - **Default: inline code, not a link** (`` `src/.memoria/engines/ingest/...` ``). A docs reader rarely needs to open the file, and a code span is immune to both the site-exclusion problem and the §7 rename churn.
  - **Only if a clickable link is genuinely warranted:** use an **absolute GitHub blob URL** (`https://github.com/eranroseman/memoria-vault/blob/main/src/...`) — the only form that resolves from the published site. (Pins to `main`; will drift if the file moves.)
  - Sweep all existing `../…/src/…` links and convert them. Reserve relative links for *other docs pages* (which ARE on the site) and ADRs.
- **[DONE] Guardrail added.** `docs-doctor` now has `check_site_local_links`, which blocks any *published* page linking outside the site (the `src/` case). The convention is also documented in the **docs-review** playbook (§2). Filesystem-only link checks otherwise pass these dead-on-site links silently — that's what this check catches.
- **[DONE] The sweep + promotion.** The existing published-docs source links were converted (default: inline code; absolute `github.com/.../blob/main` URL where a click is warranted — see the rename caveat in §7), and `check_site_local_links` now reports errors so the rule can't regress. Spot-check the rendered Pages site after.
- **[ADR] ADR-link policy.** Decide whether inline ADR links help or distract. Recommend: **keep ADR references but demote them** — no ADR in subheadings/parentheses; instead a "Decisions" footer line per page linking the governing ADRs. This declutters the prose (the note's "subheadings include the ADR in parentheses" complaint) while preserving traceability. Follow the source-link convention in [Documentation references — source links, ADR links, and per-operation Diátaxis split](../../../adr/73-docs-reference-conventions.md): link text must be the page title, not a filename — keep that when reworking links.
