...
- **[DONE] Guardrail added.** `docs-doctor` now has `check_site_local_links`, which flags any *published* page linking outside the site (the `src/` case) as an **advisory warning** (currently 77 advisories surfacing all ~70 links across 17 files; exit stays 0 so CI is unaffected). The convention is also documented in the **docs-review** skill (§2). Filesystem-only link checks otherwise pass these dead-on-site links silently — that's what the advisory catches.
- **[FIX — open] The sweep + promotion.** Convert the ~70 existing links (default: inline code; absolute `github.com/.../blob/main` URL where a click is warranted — see the rename caveat in §7), then **promote `check_site_local_links` from warning to error** so the rule can't regress. Spot-check the rendered Pages site after.
