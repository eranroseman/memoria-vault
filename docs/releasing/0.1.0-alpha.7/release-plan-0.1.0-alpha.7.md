...
navigation, and the Memoria-tuned Obsidian config — the buildable, sandbox-verified
subset of the UI redesign. The design is settled across three `tmp/` design docs and
verified live against Obsidian 1.12.7; the single biggest thing between here and cut is
that **none of it is an ADR or built yet** — the net-new persistent shell + Portals are
the long pole, and two items gate scope: one **MUST-VERIFY** (`newLinkFormat: absolute`
vs the verified backlink behavior) and one **release decision** (day-1 empty-state).
...
(Inbox/Library/Knowledge/Project) switched by a zero-plugin nav row, *retiring* the
ADR-68 workspace-swap mechanism (`load-workspace.js`, the QuickAdd workspace macros, and
Workspaces Plus) for one reset workspace; **Portals** as gated folder-navigation chrome;
and the **Memoria-tuned `src/.obsidian/` config + CSS** legibility tier. It **closes**
the shipped-UI issues #659 (one title), #665 (de-duplicated Inbox), #666 (workspace
...
| G3 | **Persistent shell & gates (ADR-68 amended).** Four dashboard notes (Inbox/Library/Knowledge/Project) embed their §3 views; the nav-row switch reuses the active tab; Homepage opens Inbox; `load-workspace.js` + the QuickAdd workspace macros are retired for one "Memoria" reset workspace; Workspaces Plus is not bundled. | S3 + S5 | Gate {{ #NN }}; #666 |
| G4 | **Portals navigation, gated (ADR-74).** A vendored Portals `data.json` (six folder portals; `system/`+`.memoria` Hidden; `_Tag Notes` relocated) ships via `replaceFileExplorer: true` with the core `file-explorer` kept enabled; Portals is added to `community-plugins.json` + `plugin-provenance-lock.json` with sha256. | S0 + S3 + S5 | Gate {{ #NN }}; #667, #663 |
...
| S2 | dry-run: installer substitution carries the four dashboards, `app.json`, the Portals `data.json`, and the snippet enablement; golden-copy reconciliation applies `appearance.json` (#668) |
| S3 | integration (Obsidian, sandbox): the §3 Bases render their views; gate dashboards embed `![[base#View]]`; the nav-row switch reuses the active tab; the Portals navigator renders the folder portals with `system/` hidden |
...
in the [Memoria Issue Tracker](https://github.com/users/eranroseman/projects/1). Two
hard facts this release: **the `newLinkFormat: absolute` re-verification (G1/S4) is the
one finding that can silently invalidate orphan/backlink views** — run it first; and
**the day-1 empty-state (G6) is a release decision, not a build task** — decide it before
the dashboards are authored, because it shapes their copy.

## 5. Out of scope (deferred)

Deliberately later, each for a named reason (the per-surface deferred set lives in
[`tmp/ui-architecture-future.md`](tmp/ui-architecture-future.md)):
...
3. **ADR status maintenance.** Accept the new UI ADR that **amends ADR-68** (gate set
   ...
   Portals plugin; note ADR-70 consistency. Run the retire-sweep (expected no-op).
...
6. **Delete `tmp/`** design notes (the four design docs + the exec plan) — promote the
   durable decisions into the UI ADR first.
