...
1. **Choose a deployment pattern.** The adopted `local-only` default and the common conventions are in [deployment options](../../explanation/deployment/deployment-options.md); the multi-machine patterns and their full comparison are the [multi-machine deployment proposal](../../adr/63-multi-machine-deployment.md). `local-mesh` (Syncthing, peer-to-peer) suits most setups; `always-on` (Syncthing + VPS) is for unattended overnight ingest. Pick based on whether the secondary device is always reachable.
