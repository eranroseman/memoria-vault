...
Settle the `research_area` (and `methodology`) the ingest engine couldn't decide on its own ([Ingest routing](../../reference/ingest.md)). Three things land on you — the rest is applied automatically:
