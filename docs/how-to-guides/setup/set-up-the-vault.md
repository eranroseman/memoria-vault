...
All seven `memoria-*` profiles appear in the output. If a profile is missing, the script reported that its required files weren't present — check [Implementation status](../../../project-files/implementation-status.md) for what's currently wired.
...
- Implementation status: [Implementation status](../../../project-files/implementation-status.md)
