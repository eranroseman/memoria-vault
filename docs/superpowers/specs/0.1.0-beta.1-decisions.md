...
and whole-tree conformance checking. Four deliberate omissions, each
additive later at zero migration cost: `status` (absent means stable per
spec; staging is the draft plane; `superseded`/`archived` carry
deprecation), `stale_after` (no TTL semantics exist yet), the Attested
Computation type schema (no instances exist; the type registry's DB
parity makes it a real change, and the spec tolerates unknown types), and
`log.md` (optional per §9; it would render from the journal that
regeneration itself appends to, so it can never satisfy the
tracked-projection drift contract). `timestamp` joins
...
## 2026-08-02 - Model-call cost telemetry: the six design decisions are ratified

Y: The six decisions the model-call cost-telemetry design left marked "made
here; confirm at review" are ratified as written, unchanged: (1) the three
existing `model_call` journal events gain `usage` / `cost_usd` / `elapsed_s`
rather than a new event type; (2) `_pydantic_ai_chat` returns a plain
`{text, usage, cost_usd, elapsed_s}` dict, not a dataclass or typed model;
(3) `cost_usd` is a nullable best-effort estimate — null for unpriced models,
never fabricated, and never an input to the token breaker; (4) telemetry
carries integers and floats only, never prompt or completion text; (5)
fixture-backed model paths report `usage=None` rather than synthesising
numbers; (6) no new dependency and no `SCHEMA_VERSION` bump.

Because: the alpha.22 COST tranche shipped against these defaults and its gate
is green, but Step 0 was ticked by the execution run itself on the reasoning
that a standing directive to execute the plan ratifies the plan's own
defaults. Review correctly called that self-certification — the design asked
for confirmation *at review*, and a reviewer is not the PI. Nothing was
blocked; what was missing was a real ratification rather than an inherited
one. Independent verification against the pinned SDK confirmed all six are
implementable as written, that `genai_prices.get_snapshot()` reads a bundled
snapshot with no network I/O (so the offline perimeter holds), and that the
payload captures no content. Decisions 3 and 4 are the two that would matter
if wrong, and both fail safe.

Pointers:
- What shipped: `design-history/22-alpha.22.md` §3 (COST). The owning design
  record was retired in #1726; the frozen chapter is the surviving statement.
- Consumers: the three `model_call` journal literals and the model-call
  reference page.

Status: ratified for beta.1 (decided 2026-08-02, after the tranche shipped;
no decision changed, so no rework follows). Closes #1561.
