...
- [x] A clean, tracked bundle Concept receives its first `file_baseline` during an observe sweep; each baseline-tracked Concept is compared on later sweeps even when an external Git commit leaves the worktree clean.
- [x] A journal-backed writer commit refreshes affected bundle baselines from the committed Git bytes. A generic commit without a matching journal hash cannot bless its changed bytes, so a later sweep witnesses it.
...
- [x] Reproduce the Pandoc raw-format and file-baseline witness gaps with focused regressions.
- [x] Repair Markdown neutralization, stable observed snapshots, baseline coverage, and journal-backed baseline refresh.
- [x] Run focused writer, content-security, runtime-state, capability-index, and worker-integrity coverage.
- [x] Run the full repository gate (`501 passed, 8 skipped, 800 deselected`; offline smoke and static gates green) and final security diff review (no reportable findings).
