...
- **Seven specialist Hermes profiles**, each with narrow permissions and a clear exit condition (explicitly avoiding "one model does everything"). *The exact roster evolved* — the Old Skeleton names coder/writer/socratic/mapper/librarian/verifier/cmdr; by the alpha.1 cut the shipped set had consolidated to **five** — co-pi, engineer, librarian, peer-reviewer, writer (retriever-scout, proposed in ADR-37, was superseded by ADR-48 and never shipped).
