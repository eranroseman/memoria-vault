---
topic: explorations
title: Multi-vault and multi-machine
status: deferred
created: 2026-05-31
parent: Design notes
...
**What.** `hermes profile export` / `import` snapshots carrying `state.db` chat history between machines, enabling session continuity across devices without a shared memory server. Extends the `memories/` junction sync pattern in [deployment options](../explanation/deployment/deployment-options.md).
