...
**Extraction mechanism.** For papers whose method or outcome is carried by a figure or table rather than prose, Hermes's `vision_analyze` reads the figure/table directly — making `_aspects.method` / `_aspects.outcome` figure-informed rather than abstract-only. It is available now but not wired into the v0.1 ingest pipeline; this proposal is the natural place it gets adopted (the [ingest reference](../reference/ingest.md) notes it as an available extraction path).
