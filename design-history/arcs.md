# Part IV — Synthesis: evolution arcs (alpha.1 -> alpha.18)

## Evolution arcs (alpha.1 -> alpha.18)
...
**Current (as of alpha.18):** No agent autonomy is part of the product baseline; models and agents may propose, but the engine owns writes, Guard owns egress, `VaultWriter` owns machine file changes, and alpha.17/18 work still routes through operations rather than direct agent file authority.
...
**Current (as of alpha.18):** The Memoria-born vault root uses file-backed homes `notes/`, `hubs/`, `projects/`, `digests/`, and `fulltext/`, while catalog Work records and lifecycle/verdict state remain database state; project `outline.md`/`draft.md` artifacts live under the project rather than encoding state in folders.
...
**Current (as of alpha.18):** The model keeps human `note`/`hub`/`project` Concepts, keeps catalog Works in SQLite, stores source-derived files as `digests/<work_id>.md` and `fulltext/<work_id>.md`, and folds human Work interpretation into `note` with `mode: work`; draft write-back creates ordinary unchecked notes, not a new concept type.
...
**Current (as of alpha.18):** Frontmatter remains meaning-only; Memoria-born markers identify machine material, while verdicts, derived state, checks, and run records live outside authored prose. Evidence sets remain body markers plus rebuildable SQLite rows, and alpha.18 rejects retired frontmatter copies such as `evidence_set`, `citations`, `citekey`, and `project`.
...
**Current (as of alpha.18):** OKF remains an interchange/keep-test boundary, not the internal representation; `fulltext` is an OKF-like frontmatter label for generated reproductions, while the runtime's DB Concept set stays narrower. Draft export is a verified project output path, not a full publication/interchange release gate.
...
**Current (as of alpha.18):** Hermes is not part of the runtime; the process model is a standalone engine plus on-demand `memoria serve`, CLI/read-API operations, and credential-free workers behind Guard.
...
**Current (as of alpha.18):** SQLite owns catalog Work records, derived indexes, checks, runs, jobs, costs, incidents, Guard logs, provider payload metadata, `event_log`, and rebuildable `evidence_sets`; markdown/PDF/BibTeX own the human keep-set and survival artifacts, with evidence markers as the canonical draft-side source for evidence-set rows. The runtime schema is `user_version = 7` and legacy DB versions fail fast.
...
**Current (as of alpha.18):** Capture is land-first and provider-provenanced; Zotero/DOI/PDF/GitHub/local sources land immutable artifacts or visible triage stubs, enrichment still never silently merges identities, Work identity is `work_id`, and venue edges are `published_in`. Gap analysis free-joins existing graph edges for authorship, institutions, and venues without adding an entity table.

**Pending (unreleased):** Future discovery and query work must decide the minimum provider/discovery set and derived query substrate beyond alpha.18's normalized catalog graph.
...
**Current (as of alpha.18):** The required surface remains engine/read-API first; slice, compose, verify, export, exploration, write-back, and normalized catalog/Work operations all have host-neutral CLI/read-API paths. A future Obsidian plugin is recommended as a thin rendering adapter, not shipped alpha.18 scope.
...
**Current (as of alpha.18):** Alpha.18 is the current closed checkpoint technical baseline; release close requires a frozen chapter, updated arcs, an updated latest-checkpoint marker in `design-history/README.md`, and deletion of the scratch release workspace after durable routing.
