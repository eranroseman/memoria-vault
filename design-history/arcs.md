...
**Pending (unreleased):** Future output work must keep proving crash/rebuild behavior on the full read/write path, and the runtime CI gate must be widened to the `runtime` level once its environment-sensitive tests are hardened.
