...
def coherence(text: str, pages: int = 1) -> dict:
    ...
    word_ratio = (len(tokens) and sum(1 for t in tokens if len(t) <= 30) / len(tokens)) or 0.0
...
def from_pdf(path: Path) -> tuple[str | None, str]:
    ...
    print(f"[extract] PDF parse failed for {path}: rc={proc.returncode}: {last}", file=sys.stderr)
...
def extract(ids: dict, pdf_path: str | None = None, email: str = "", api_key: str = "") -> dict:
    ...
    if pdf_path:
        ...
        if t:
            ...
            return {
                "text": t if c["ok"] else "",
                "source": "pymupdf4llm" if c["ok"] else "needs-ocr",
                "chars": c["chars"],
                "degraded": not c["ok"],
                "coherence": c,
                "note": note,
...
def main() -> int:
    ...
    r_preview = {
        **r,
        "text": (r["text"][:300] + "...") if len(r.get("text", "")) > 300 else r.get("text", ""),
    }
