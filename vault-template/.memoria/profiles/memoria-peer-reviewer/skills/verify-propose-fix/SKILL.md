...
description: "When a verification finding has an obvious remedy, propose the fix as a worker-owned candidate attention projection — the exact edit, spelled out, with the ADR-51 honesty body — WITHOUT touching the thing under review (flag, don't fix). Used after verify-check-citation / verify-trace-claim when a finding is mechanical enough that the remedy is clear (a typo'd citekey, a wikilink to the wrong note, a stale superseded reference)."
version: 1.0.0
author: Memoria
license: MIT
...
decision, not a research task. The fix is proposed as worker-owned attention — the
thing under review is never edited; the accepted fix is applied by the PI or delegated
to the owning lane.
...
one matching target; a citation to a source whose claim-bearing note was superseded, where the
...
3. **Request attention.** One `candidate` attention projection per reviewed target,
   listing each proposed edit and linking the originating `flag` attention item. One
   item per review pass, never one per edit (ADR-54).

## Output contract

A `candidate` attention request:
...
- Never apply the fix — not even a one-character citekey edit; the attention request
  is the entire output (separation of duties, ADR-48).
- Never propose fixes for judgment findings (an unsupported claim has no mechanical
  remedy — that is `verify-card-gap`'s lane).
- The PI's rejection of a fix suggestion is information: do not re-propose the same fix
  on the next pass; note the standing rejection instead.
